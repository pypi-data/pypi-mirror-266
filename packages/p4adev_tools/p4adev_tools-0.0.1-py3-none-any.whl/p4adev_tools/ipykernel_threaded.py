...
def init_ipython_kernel(**kwargs) -> IPyWrapper:
	cn_info: dict = kwargs.pop('cn_info', None)
	picbck: Callable[[IPyWrapper], None] | None = kwargs.pop('post_init_callback', None)
	update_conf: Callable[[Config], None] | None = kwargs.pop('update_conf', None)

	kernel_wrapper = IPyWrapper(**kwargs)

	kernel_wrapper._connection_info = cn_info  # noqa: SLF001
	kernel_wrapper._post_init_callback = picbck  # noqa: SLF001
	kernel_wrapper._update_conf = update_conf  # noqa: SLF001

	kernel_wrapper.start()
	return kernel_wrapper


def update_conf(conf: Config) -> None:
...
def ipython_kernel_thread(conf_opts: dict[str, Any] = {}) -> IPyWrapper:  # noqa: B006
	# TODO: Log properly..
	from logging import getLogger, INFO

	ipykl = getLogger('p4adev_tools.ipyk')
	ipykl.setLevel(INFO)

	ipykl.info('Preparing..')
	...
	ipykl.info('Started thread!')

	return kernel_wrapper


if __name__ == '__main__':
	ipython_kernel_thread().thread.join()
