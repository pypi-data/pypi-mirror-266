...
from NuGigSkillNER.matcher_class import Matchers, SkillsGetter
from NuGigSkillNER.utils import Utils
from NuGigSkillNER.general_params import SKILL_TO_COLOR
...
class SkillExtractor:
    ...
    def annotate(
        ...
        to_process = skills_low_form + skills_uni_full
        process_n_gram = self.utils.process_n_gram(to_process, text_obj)

        doc = self.skill_getters.get_lemmed_text(text_obj)

        return {
            'lemmed text': str(doc),
            'original text': text_obj.immutable_text,
