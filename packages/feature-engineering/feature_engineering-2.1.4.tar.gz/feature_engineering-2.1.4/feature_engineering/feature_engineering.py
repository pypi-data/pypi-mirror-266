...
class FeatureEngineering(BaseEstimator, TransformerMixin):
    def __init__(self, df, target, numeric_columns=None, categorical_columns=None, date_columns=[], handle_outliers=None,
        ...
        self.target_variable_final = None
    ...
    def select_features(self):
        """Selects features based on the specified selection strategy, using the potentially modified target variable."""
        ...
        if self.min_class_size is not None:
            ...
            # Dynamically calculate min_class_size and n_splits if not provided
            min_class_size = min(self.target_variable_final.value_counts())  # Use the potentially modified target variable
        ...
        if self.selection_strategy == 'embedded':
            ...
            selector = SelectFromModel(model).fit(self.X_preprocessed, self.target_variable_final)
            ...
            selector = RFECV(estimator, step=1, cv=StratifiedKFold(n_splits=n_splits),
                            ...
                            min_features_to_select=self.n_features_to_select or 1).fit(self.X_preprocessed, self.target_variable_final)
        else:
    ...
    def select_embedded_method(self, n_splits=5):
        ...
        for model in models.values():
            score = cross_val_score(model, self.X_preprocessed, self.target_variable_final, cv=n_splits, scoring='accuracy' if self.task == 'classification' else 'r2').mean()
        
            # Now, apply hyperparameter tuning if specified
            tuned_model = self.perform_hyperparameter_tuning(model, self.X_preprocessed, self.target_variable_final)
    ...
    def run(self):
        ...
        if self.sampling_strategies:
            ...
            self.target_variable_final = self.y
        else:
            # If no sampling strategies were applied, use the original target column
            self.target_variable_final = self.df[self.target]

        # Step 3: Perform feature selection
        # Step 3: Perform feature selection with the potentially modified target variable
        self.select_features()

        # Concatenate preprocessed features with the target variable for the final DataFrame
        df_final = pd.concat([self.X_preprocessed, self.target_variable_final], axis=1)
