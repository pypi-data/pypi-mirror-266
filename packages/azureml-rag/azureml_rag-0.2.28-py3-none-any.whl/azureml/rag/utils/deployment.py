...
def infer_deployment(aoai_connection, model_name):
    ...
    try:
        ...
        raise ValueError(
            "Could not parse subscriptions from workspace_details"
        ) from e
    try:
        resource_group = workspace_details["resourceGroups"]
    except Exception as e:
        raise ValueError(
            "Could not parse resourceGroups from workspace_details"
        ) from e
    ...
    if aoai_name is None:
        try:
            ...
            raise ValueError(
                "Could not parse target from aoai_connection"
            ) from e
        try:
            aoai_name = target[target.find('https://') + len('https://'): target.find('.openai.azure.com')]
        except Exception as e:
            raise ValueError(
                "Could not parse aoai name from aoai_connection[\"properties\"][\"target\"]"
            ) from e
        if aoai_name is None:
    ...
    if version.parse(openai.version.VERSION) >= version.parse("1.0.0"):
        ...
        print(f"calling cognitive service with subscription_id is {subscription_id}, resource_group is {resource_group}, aoai_name is {aoai_name}")
        client = CognitiveServicesManagementClient(credential=DefaultAzureCredential(process_timeout=60), subscription_id=subscription_id)
        deployment_list = convert_to_dict(client.deployments.list(resource_group_name=resource_group, account_name=aoai_name))
        result = []
