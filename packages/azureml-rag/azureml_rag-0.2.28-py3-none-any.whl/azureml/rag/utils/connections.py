...
) -> Union[dict, WorkspaceConnection, Connection]:
    """
    ...
    if client == "sdk" and MLClient is not None and packages_versions_for_compatibility["azure-ai-ml"] >= "1.10.0":
        ...
        try:
            ...
            if connection.credentials is None:
                raise Exception("WorkspaceConnection did not parse credential object")
...
def get_id_from_connection(connection: Union[dict, WorkspaceConnection, Connection]) -> str:
    ...
    elif isinstance(connection, WorkspaceConnection) or isinstance(connection, Connection):
        return connection.id
    else:
...
def get_connection_by_name_v2(workspace, name: str, credential: Optional[TokenCredential] = None) -> dict:
