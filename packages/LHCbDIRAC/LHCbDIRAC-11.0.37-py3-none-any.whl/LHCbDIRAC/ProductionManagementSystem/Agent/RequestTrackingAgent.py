...
class RequestTrackingAgent(AgentModule):
    ...
    def execute(self):
        ...
        update = []
        if result["OK"]:
            gLogger.verbose(f"Requests tracked: {','.join([str(req['RequestID']) for req in result['Value']])}")
            for request in result["Value"]:
                result = self.bkInputNumberOfEvents(request)
                if result["OK"]:
                    update.append({"RequestID": request["RequestID"], "RealNumberOfEvents": result["Value"]})
                else:
                    gLogger.error(f"Input of {str(request['RequestID'])} is not updated: {result['Message']}")
        else:
            gLogger.error(f"Request service: {result['Message']}")
        if update:
    ...
    def bkInputNumberOfEvents(self, request):
        ...
        try:
            condition = {
                "ProcessingPass": str(request.get("inProPass", "")).strip(),
            ...
            if request.get("inSMOG2State", None):
                condition["SMOG2State"] = request["inSMOG2State"]
        except KeyError as ke:
            gLogger.error(f"{request['RequestID']} is incomplete: {repr(ke)}")
        ...
        try:
            ...
            return S_ERROR(f"Can not convert result from BK call: {str(e)}")
        return S_OK(sum_nr)
