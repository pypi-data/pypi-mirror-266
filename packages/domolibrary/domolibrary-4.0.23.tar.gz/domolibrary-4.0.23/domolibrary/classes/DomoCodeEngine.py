...
class DomoCodeEngine_Function:
    ...
    @classmethod
    def _from_json(cls, obj, language, package_id, version):
        return cls(**obj, language=language, package_id=package_id, version=version)

    def __eq__(self, other):
        if not isinstance(other, DomoCodeEngine_Function):
            return False
        
        if self.version != other.version or self.package_id != other.package_id or self.name != other.name:
            return False
...
def export(self, file_name : str = None, output_folder: str = "EXPORT/code_engine/", debug_prn: bool = False):
    output_folder = f"{output_folder}/" if not output_folder.endswith('/') else output_folder 
