...
@patch_to(DomoRoles, cls_method = True)
async def get_default_role(cls, auth : dmda.DomoAuth,
                           return_raw: bool = False,
                           debug_api : bool = False,
                           debug_num_stacks_to_drop = 2,
                           session: httpx.AsyncClient = None
                          ):
    res = await role_routes.get_default_role(auth=auth, session = session,
                                             parent_class = cls.__name__,
                                             debug_num_stacks_to_drop = debug_num_stacks_to_drop,
                                             debug_api = debug_api)
    if return_raw:
    ...
    return await DomoRole.get_by_id(role_id = res.response, auth = auth,
                                    debug_api = debug_api,
                                    # parent_class = cls.__name__,
                                    session = session,
                                    debug_num_stacks_to_drop = debug_num_stacks_to_drop)
