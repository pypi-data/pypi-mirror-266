...
def sessionStateSet():
    with st.session_state["set_cookie_placement"][0]:
        ...
        sessionS.setItem(keyItem, keyValue)
...
with cols[0].form("set_storage"):
    ...
    st.form_submit_button("set this", on_click=sessionStateSet)
...
if st.session_state["get_storage_item"] != "":
    get_item = sessionS.getItem(itemKey=st.session_state["get_storage_item"])
    st.write(get_item)
...
# if st.session_state["get_storage_item"] != "":
#     get_item = sessionS.getItem(st.session_state["get_storage_item"])
#     st.write(get_item)


# def deleteItem():
#     with st.session_state["delete_cookie_placement"][0]:
#         sessionS.deleteItem(st.session_state["delete_storage_item"])
