...
class PwpTester:
    ...
    def program_31(self):
        ...
        # ----------------------------------------------------
        self.process({'--enable-rename': 'false'})
        # ----------------------------------------------------
    ...
    def program_33(self):
        ...
        # ----------------------------------------------------------------
        self.process({'--trust-date-in-filename': 'false'})
        # ----------------------------------------------------------------
    ...
    def program_34(self):
        ...
        # ----------------------------------------------------------------
        self.process({'--dryrun': 'NO-VALUE-EXPECTED'})
        # ----------------------------------------------------------------
    ...
    def program_207(self):
        ...
        # ------------------------------------------------------------------
        self.process({'--verify-album': '2023/2023-06-Juin-17-Armor-cup',
                      '--triage': None})  # noqa})
        # ------------------------------------------------------------------
    ...
    def program_300(self):
        ...
        self.process({'--triage': None,
                      ...
                      '--verify-album-2': "2021",
                      '--verify-album-3': "2023"},
                     msg="Verify albums 2020,2021,2023 without database")
        # -----------------------------------------------
    ...
    def program_906(self):
        ...
        LOGGER.test_msg(f'--------------- starting {mn}')
        LOGGER.test_msg('')
        LOGGER.test_msg('auto-test de install --exe')

        args = [sys.executable, os.getcwd() + r"\installer\installer.py",
                "--self-test", "--exe", "--chdir", os.getcwd() + "/tests/results"]

        ret = subprocess.run(args, capture_output=True)  # cwd= does not work !

        if ret.returncode != 0:
            LOGGER.warning(f'return code {ret.returncode}')

        self.assert_file("tests/results/ffmpeg.exe")
        self.assert_file("tests/results/ffprobe.exe")
    ...
    def program_907(self):
        ...
        LOGGER.test_msg(f'--------------- starting {mn}')
        LOGGER.test_msg('')
        LOGGER.test_msg('auto-test de install --python')

        args = [sys.executable, os.getcwd() + r"\installer\installer.py",
                "--self-test", "--python", "--chdir", os.getcwd() + "/tests/results"]

        ret = subprocess.run(args, capture_output=True)
        if ret.returncode != 0:
            LOGGER.warning(f'return code {ret.returncode}')

        self.assert_file("ffmpeg.exe")
        self.assert_file("ffprobe.exe")
        self.assert_file("mariaDB.msi")
