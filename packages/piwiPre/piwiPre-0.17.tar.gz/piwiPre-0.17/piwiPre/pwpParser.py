...
3) Dans le répertoire personnel de l'utilisateur (HOME), c'est le fichier .piwiPre.ini qui est lu. 
   ...
   - remote-host remote-port  remote-user remote-pwd remote-incoming
   - piwigo-user 
...
""")  # noqa
        ...
        self.add_item('install-tools',
                      help="installs FFMPEG and MariaDb, that are windows tools used by the Python version",
                      fr_help="Installe FFMPEG et MariaDb, qui sont des outils windows utilisés par la version Python",
                      config=f"""
install-tools works only for Windows systems, and is useful when using the PYTHON version of piwiPre

- It installs FFMPEG in {os.environ['PROGRAMFILES(X86)']}/ffmpeg    
- It downloads the mariaDb MSI installer, to be executed as an administrator by the user       

For a complete self-install, it is recommended to also run piwiPre --reset-ini triage (or default/local/remote...)
in the working directory

For the installation of the EXE version, see --install-exe below       
""",
                      fr_config=f"""
install-tools fonctionne uniquement sous windows, et est utile pour l'usage de la version PYTHON de piwiPre,

- installe FFMPEG dans {os.environ['PROGRAMFILES(X86)']}/ffmpeg 
- télécharge l'installateur MSI de MariDB, à exécuter en tant qu'administrateur de la machine


Pour une auto-installation complète, il est recommandé d'exécuter piwiPre --reset-ini triage (ou default/local/remote)
dans le répertoire de travail
""",
                      action='store_true',
                      location='args')

        self.add_item('install-exe',
                      help="installs the LATEST windows executable",
                      fr_help="installe le programme windows LE PLUS RECENT",
                      config=f"""
install-exe works only on Windows systems.

- It installs the latest EXE file of piwiPre, in {os.environ['PROGRAMFILES(X86)']}/piwiPre,
- You may want to add it the the PATH, so that you just have to run "piwiPre" from whatever directory.
- It installs FFMPEG in {os.environ['PROGRAMFILES(X86)']}/ffmpeg
...
""",
                      fr_config=f"""
install-exe ne fonctionne que sous Windows.
Il installe la dernière version de l'exécutable dans {os.environ['PROGRAMFILES(X86)']}/piwiPre,
et le rajoute dans le PATH, pour qu'il suffise d'exécuter 'piwiPre' dans n'importe quel répertoire.
...
""",
        ...
        self.add_item('test-install-tools',
                      help="test of install-tools",
                      fr_help="test de install-tools",
                      action='store_true',
                      location='args')

        self.add_item('test-install-exe',
                      help="test of install-exe",
                      fr_help="test de install-exe",
                      action='store_true',
                      location='args')
...
=======================""", lang='fr')
        ...
        self.add_item('triage',
                      help='Sets the TRIAGE directory where are stored incoming pictures/videos.',
                      fr_help="Indique le nom du répertoire de tri où sont rangées les images/vidéo importer",
                      action='store',
                      default='TRIAGE',
                      config="""
...
""")
        ...
        self.add_item('album',
                      ...
                      fr_help="Indique le répertoire racine de l'ALBUM où les images sont stockées",
                      action='store',
                      default='ALBUM',
                      config="""
...
""")
        ...
        self.add_item('piwigo-album-name',
                      help="Sets the name of the root piwigo album managed",
                      fr_help="Indique le nom de l'album racine de piwigo à gérer",
                      action="store",
                      default=None,
                      config="""
...
""")
        ...
        self.add_item('backup',
                      help='Sets the backup directory where modified files are saved.',
                      fr_help="Indique le répertoire de backup où sont sauvegardés le fichiers modifiés.",
                      action='store',
                      default='BACKUP',
                      config="""
...
""")
        ...
        self.add_item('web',
                      ...
                      fr_help="Indique le répertoire racine pour les miniatures (thumbnail) de piwigo",
                      action='store',
                      default='WEB',
                      config="""
...
""")
        ...
        self.add_item('remote-web',
                      ...
                      fr_help="Indique le répertoire DISTANT (c-a-d sur le serveur distant) pour les miniatures",
                      action='store',
                      default=None,
                      config="""
...
""")
        self.add_item('remote-album',
                      help='Sets the REMOTE directory for piwigo albums.',
                      fr_help="Indique le répertoire DISTANT (c-a-d sur le serveur distant) pour les photos",
                      action='store',
                      default=None,
                      )
...
""")
        ...
        self.add_item('remote-pwd',
                      help='Sets the password of the piwigo remote-user ',
                      fr_help="Indique le mot de passe de remote-user",
                      action='store',
                      default=None,
                      location='config')
...
""")
        ...
        self.add_item('remote-port',
                      ...
                      fr_help="indique le port ssh du serveur distant distant",
                      action='store',
                      pwp_type=int,
                      default=42)
...
- Valeur = 'incoming' est une valeur typique""")
        ...
        self.add_item('piwigo-user',
                      ...
                      fr_help="nom d'utilisateur piwigo pour l'accès aux phots/vidéo/répertoires",
                      action='store',
                      default=None)
    ...
    - value = true: metadata is written if different from what was already in the file 
      (which includes no value)""")
        ...
        self.add_item('enable-pwg-representative',
                      ...
                      fr_help="autorise la creation de l'image JP qui représente une vidéo dans piwigo",
                      action='store',
                      choices=['true', 'false'],
                      default='true')
        ...
        self.add_item('ffmpeg-path',
                      ...
                      action='store',
                      default=(os.environ['PROGRAMFILES(X86)'] + '/ffmpeg/bin/') if platform.system() == "Windows"
                      else '/usr/bin/',
                      config="""
...
def build_official_rst(autotest: bool):
    if autotest:
        filename_en = "tests/results/configuration.rst.autotest"
        filename_fr = "tests/results/configuration_fr.rst.autotest"
    else:
