...
class PwpActor:
    ...
    #     if platform.system() != "Windows":
    #         return
    #     elevate(cwd=LOGGER.cwd)
    #     try:
    #         with winreg.OpenKey(winreg.HKEY_CURRENT_USER, "Environment", access=winreg.KEY_SET_VALUE) as env:
    #             current = winreg.QueryValueEx(env, "Path")[0]
    #             LOGGER.msg(f"Current PATH = '{current}'")
    #             if value not in current:
    #                 current += ";" + value
    #                 winreg.SetValue(env, "Path", winreg.REG_EXPAND_SZ, current)
    #                 LOGGER.warning(f"Added '{value}' to PATH environment variable")
