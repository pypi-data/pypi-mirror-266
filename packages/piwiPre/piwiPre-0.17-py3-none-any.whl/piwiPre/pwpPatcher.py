...
class PwpPatcher:
    ...
    def update_from_source(self, filename, source, autotest: bool):
        ...
        if not autotest and os.path.getmtime(filename) > os.path.getmtime(source):
            print(f"msg     file '{filename}' is older than source '{source}': patch useless")  # pragma: no cover
            return                                                                              # pragma: no cover
        ...
        if autotest:
            ...
            new_file = filename + '.new'                                                         # pragma: no cover
        old_file = filename + '.bak'
    ...
    def run(self):
        ...
        self.update_file('piwiPre/pwpVersion.py',
                         {r"help = '([^']*)'": f"help = '{self.spec} at {self.date}'",
                          ...
                          r"installer = '([^']*)'": f"url = '{url_base}/{self.spec}/piwiPreInstaller-{self.spec}.exe'"
        ...
        prologue2_fr = "- L'installateur Windows sur le serveur d'artefacts gitlab:"
            ...
            self.update_file('source/download.rst',
                             {
                                 ...
                                 r".*Windows instal.*": f"{prologue2_en} `piwiPreInstaller-{self.spec}.exe " +  # noqa
        ...
        # TODO: est ce que on change PwpVersion dans autotest????? à éviter !!!!
        if self.args.autotest:
        ...
        if self.args.exe:
            ...
            res = os.system("python -m PyInstaller -y installer/installer.py --onefile --name piwiPreInstaller")  # noqa
            print(f"msg     DONE: build exe piwiPreInstaller -> {res}")
