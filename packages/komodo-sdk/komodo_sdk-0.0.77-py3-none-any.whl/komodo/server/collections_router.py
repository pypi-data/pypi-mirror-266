...
from komodo.server.globals import get_appliance, get_appliance_runtime, get_email_from_header
...
async def get_collection(shortcode: str, email=Depends(get_email_from_header),
                         appliance=Depends(get_appliance)):
    collection = get_collection_from_disk(shortcode, email, appliance)
    return collection.get_collection_with_files()


@router.delete('/{shortcode}')
async def delete_collection(shortcode: str, email=Depends(get_email_from_header),
                            appliance=Depends(get_appliance)):
    collection = get_collection_from_disk(shortcode, email, appliance)
...
async def upload(shortcode: str, files: List[UploadFile] = File(...),
...
async def upload_stream(request: Request, shortcode: str, email=Depends(get_email_from_header),
                        appliance=Depends(get_appliance)):
...
def download_file(file_guid: str, shortcode: str, email=Depends(get_email_from_header),
                  appliance=Depends(get_appliance)):
    print("download_file: ", file_guid, shortcode, email, appliance.shortcode)
    collection = get_collection_from_disk(shortcode, email, appliance)
...
def download_file_format(file_guid: str, format: str, shortcode: str, email=Depends(get_email_from_header),
                         appliance=Depends(get_appliance)):
    collection = get_collection_from_disk(shortcode, email, appliance)
    file = collection.get_file_by_guid(file_guid)
    if not Path(file).exists():
...
async def delete_file(file_guid: str, shortcode: str, email=Depends(get_email_from_header),
                      appliance=Depends(get_appliance)):
    collection = get_collection_from_disk(shortcode, email, appliance)
    file = collection.get_file_by_guid(file_guid)
    if collection.belongs_to_user():
...
def get_collection_from_disk(shortcode, email, appliance) -> KomodoCollection:
