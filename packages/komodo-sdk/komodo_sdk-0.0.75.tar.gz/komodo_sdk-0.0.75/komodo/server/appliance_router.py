...
def get_appliance_configuration(email=Depends(get_email_from_header), appliance=Depends(get_appliance)):
    runtime = ApplianceRuntime(appliance)
    agents = runtime.get_all_agents()
    return {
        "shortcode": appliance.shortcode,
...
def get_available_agents(email=Depends(get_email_from_header), appliance=Depends(get_appliance)):
    runtime = ApplianceRuntime(appliance)
    agents = runtime.get_all_agents()
    return {"status": "success"}
