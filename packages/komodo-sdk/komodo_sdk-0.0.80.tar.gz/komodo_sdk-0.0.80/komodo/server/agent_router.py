...
def get_runner(appliance, agent_shortcode, email, collection_shortcode=None, file_guid=None):
    ...
    user = loader.get_user(email)
    if user is None:
        raise HTTPException(status_code=401, detail="User not found: " + email)

    agent: KomodoAgent = appliance.get_agent(agent_shortcode)
    if agent is None:
        raise HTTPException(status_code=400, detail="Requested Agent is not available")

    runtime = KomodoRuntime(appliance=appliance, agent=agent, user=user)
