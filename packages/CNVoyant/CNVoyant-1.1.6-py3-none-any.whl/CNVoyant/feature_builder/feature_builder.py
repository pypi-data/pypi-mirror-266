...
class FeatureBuilder:
    ...
    def __init__(self, variant_df: pd.DataFrame, data_dir: str):
        ...
        self.temp_data_dir = os.path.join(self.data_dir, str(uuid.uuid4()))
    ...
    def remove_temp_files(self):
        ...
        def onerror(func, path, exc_info):
            ...
            If the error is due to permissions, attempt to change the permissions and retry the operation.
            """
            import stat
            if not os.access(path, os.W_OK):
                # Attempt to change the permissions and retry the deletion
                os.chmod(path, stat.S_IWUSR)
                func(path)
            else:
                raise

        shutil.rmtree(self.temp_data_dir, onerror=onerror)
