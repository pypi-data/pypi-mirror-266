...
class MotorControls(Static):
    ...
    def move_up(self):
        ...
        if self._motor_state == "enabled":
            ...
            log.write(f"We moved up ({distance_mm=}, {speed_mm_s=}, {step_mode=}).")
        else:
    ...
    def move_down(self):
        ...
        if self._motor_state == "enabled":
            ...
            log.write(f"We moved down ({distance_mm=}, {speed_mm_s=}, {step_mode=}).")
        else:
...
class SpeedControls(Widget):
    speed = reactive(DEFAULT_SPEED_STEP)
    ...
    def compose(self) -> ComposeResult:
        with Horizontal():
            ...
            yield Label(f"{DEFAULT_SPEED_STEP} mm/s", id="speed-value")

    @on(Button.Pressed, "#speed-up")
    def increase_speed(self):
        if self.speed == 1:
            self.speed = DEFAULT_SPEED_STEP
        else:
            self.speed += DEFAULT_SPEED_STEP
