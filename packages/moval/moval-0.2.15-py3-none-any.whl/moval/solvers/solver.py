                    ...
                    if flag_case_a and flag_case_b:
                        ...
                        if optimization_result.fun > search_threshold:
                            ...
                            if self.model.mode == "classification":
                                ...
                                initial_conditions_atc = [np.array([0.01 * (score_max - score_min) + score_min]), 
                                                          np.array([0.1 * (score_max - score_min) + score_min]), 
                                                          np.array([0.3 * (score_max - score_min) + score_min]), 
                                                          np.array([0.5 * (score_max - score_min) + score_min]), 
                                                          np.array([0.9 * (score_max - score_min) + score_min])]
                            else:
                                scores = []
                                for n_case in range(len(inp)):
                                    score_flatten = score[n_case].flatten() # ``n``
                                    scores.append(score_flatten)
                                score_min = np.min( np.concatenate(scores) )
                                score_max = np.max( np.concatenate(scores) )
                                initial_conditions_atc = [np.array([0.01 * (score_max - score_min) + score_min]), 
                                                          np.array([0.5 * (score_max - score_min) + score_min])]
                            results = []
                            results.append((optimization_result.fun, optimization_result.x[0]))
