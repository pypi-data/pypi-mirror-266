                ...
                for kcls in kcls_list:
                    ...
                    if flag_case_a and flag_case_b:
                        ...
                        if optimization_result.fun > search_threshold:
                            ...
                            if self.model.mode == "classification":
                                initial_conditions_atc = [np.array([np.percentile(score, 20)]), 
                                                            np.array([np.percentile(score, 40)]), 
                                                            np.array([np.percentile(score, 50)]), 
                                                            np.array([np.percentile(score, 60)]), 
                                                            np.array([np.percentile(score, 80)])]
                            else:
                                scores = []
                                for n_case in range(len(inp)):
