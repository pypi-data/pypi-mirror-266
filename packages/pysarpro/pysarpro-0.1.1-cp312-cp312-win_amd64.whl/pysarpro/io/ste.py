...
class RatFile:
    ...
    def write(self, arr=[], **kwargs):
        ...
        if 'block' in kwargs:
            ...
            if 'header' in kwargs:
                self._ioerror(
                    'The header should have been written prior to block ' 'writing!'
                )
            ...
            if arr.ndim == 2:
                ...
                offset = (
                    1000 + np.ravel_multi_index(block[::2], self.shape) * arr.itemsize
                )

            with open(self.filename, 'r+') as lun:
                # The header should be written prior block writing
                lun.seek(offset)
                arr.tofile(lun)
                lun.flush()
                self.exists = True
    ...
    def read(self, **kwargs):
        ...
        with open(self.filename, 'rb') as lun:
            ...
            arr = np.ndarray.__new__(
                np.ndarray, self.shape, dtype=self.dtype, buffer=mm, offset=offset
            )[ind]
            if self.xdrflag == 1:
                arr = arr.byteswap()
    ...
    def append(self, arr):
        ...
        if len(self.shape) > 1 and (self.shape[1:] != arr.shape[1:]):
            self._ioerror(
                'The shape specified in the header {} and the shape of '
                'the array {} don\'t correspond to each other!'.format(
    ...
    def _check_block(self, block, **kwargs):
        ...
        stop_more_than_shape = reduce(
            lambda x, y: x or y, (map(lambda x, y: (x > y), block[1::2], self.shape))
        )
        ...
        if 'arr' in kwargs:
            ...
            if block_not_shape:
                self._ioerror(
                    'Length of block components {} does not correspond to'
                    ' the shape of the array {}!'.format(
