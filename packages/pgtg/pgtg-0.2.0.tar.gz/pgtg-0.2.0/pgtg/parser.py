...
def parse_map_object(
    ...
    for tile_x in range(map_plan.width):
        for tile_y in range(map_plan.height):
            ...
            # if the tile is on the shortest path replace exit markers with subgoals
            ...
            # if the tile is in the bottom left corner replace exit markers with start
            if tile_x == 0 and tile_y == map_plan.height - 1:
                replace_features_in_tile(
                    current_tile,
                    "exit west",
                    "start",
            ...
            # if the tile is in the top right corner replace exit markers with final goals
            if tile_x == map_plan.width - 1 and tile_y == 0:
                replace_features_in_tile(
                    current_tile,
                    "exit east",
                    "final goal",
