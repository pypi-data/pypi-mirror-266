...
def main_help_text():
    ...
    upload-folder <corpus_id> <folder_path> - Uploads and indexes all documents from the specified folder to the given corpus.
...
def print_upload_folder_help():
    help_text = """
Vectara CLI Upload Folder Help
-------------------------------
The `upload-folder` command is used to upload and index all documents from a specified folder into a given corpus on the Vectara platform.

Usage:
    vectara-cli upload-folder <corpus_id> <folder_path>

Parameters:
    <corpus_id>    The ID of the corpus to which the documents will be uploaded. This is a required parameter.
    <folder_path>  The path to the folder containing the documents to be uploaded. This is a required parameter.

Examples:
    1. Upload and index documents from a folder:
       vectara-cli upload-folder 12345 /path/to/documents

Note:
- The command will process all supported document formats found within the specified folder.
- Each document will be assigned a unique document ID by Vectara if not explicitly provided.
- Ensure that the corpus ID is correct and that you have the necessary permissions to upload documents to the specified corpus.

For more information on document formats supported and other details, visit the Vectara documentation at https://docs.vectara.com.
    """
    print(help_text)
