...
class DfHelper:
    ...
    def __init__(self, **kwargs):
        ...
        parquet_keys = parquet_default_params.keys()
        parquet_options = {k: kwargs.pop(k, parquet_default_params[k]) for k in parquet_keys}
        self.parquet_options = ParquetOptions(**parquet_options)
        if self.parquet_options.use_parquet is False:
            connection_keys = connection_default_params.keys()
            connection_options = {k: kwargs.pop(k, connection_default_params[k]) for k in connection_keys}
            self.connection_config = ConnectionConfig(**connection_options)
        query_keys = query_default_params.keys()
        query_options = {k: kwargs.pop(k, query_default_params[k]) for k in query_keys}
        self.query_config = QueryConfig(**query_options)
        # The remainder of the kwargs are passed to the ParamsConfig object
        self.params_config = ParamsConfig(**kwargs)
        self.params_config.parse_params(kwargs, use_parquet=self.parquet_options.use_parquet)
    ...
    def _load(self, **options: Dict[str, Union[str, bool, Dict, None]]) -> pd.DataFrame:
        ...
        # self.params_config.filters = self.params_config.fixed_filters
