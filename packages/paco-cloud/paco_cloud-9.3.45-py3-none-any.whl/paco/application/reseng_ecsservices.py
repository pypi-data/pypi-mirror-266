...
class ECSServicesResourceEngine(ResourceEngine):
    ...
    def create_task_roles(self):
        for task in self.resource.task_definitions.values():
            # Task Role
            if task.task_role != None:
                if task.task_role.assume_role_policy == None:
                    assume_role_policy_dict = { 'effect': 'Allow', 'service': ['ecs-tasks.amazonaws.com'] }
                    task.task_role.set_assume_role_policy(assume_role_policy_dict)
                iam_ctl = self.paco_ctx.get_controller('IAM')
                iam_ctl.add_role(
                    region=self.aws_region,
                    resource=task,
                    role=task.task_role,
                    iam_role_id=task.name,
