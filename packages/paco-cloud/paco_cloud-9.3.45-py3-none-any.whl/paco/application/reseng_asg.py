...
role_name: %s""" % ("ASGInstance")
        ...
        if not hasattr(role_config, 'assume_role_policy') or role_config.assume_role_policy == None:
            ...
            if self.resource.instance_ami_type in ('amazon_ecs', 'amazon_ecs_arm'):
        ...
        if self.resource.ecs != None and self.resource.ecs.is_enabled():
            self.stack.hooks.add(
        ...
        # Disable scale-in setting when deleting ASG
        self.stack.hooks.add(
            name='DisableScaleIn.' + self.resource.name,
            stack_action=['delete'],
            stack_timing='pre',
            hook_method=self.disable_asg_scale_in,
            hook_arg=self.resource
        )
...
"""
    ...
    def disable_asg_scale_in(self, hook, asg):
        return
        asg_name = asg.stack.get_outputs_value('ASGName')
        asg_client = self.account_ctx.get_aws_client('autoscaling', self.aws_region)
