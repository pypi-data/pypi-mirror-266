...
class Route53StackGroup(StackGroup):
    def __init__(self, paco_ctx, account_ctx, route53_config, controller, model_obj):
        ...
        if self.paco_ctx.legacy_flag('route53_hosted_zone_2019_10_12') == True:
            ...
            self.init_hosted_zones(model_obj)

    def init_hosted_zones(self, model_obj):
        model_obj_ref = None
        if model_obj and model_obj.paco_ref_parts != 'resource.route53' and isinstance(model_obj, dict) == False:
            model_obj_ref = Reference(model_obj.paco_ref)
        for zone_id in self.config.get_zone_ids(account_name=self.account_ctx.get_name()):
            if model_obj_ref and model_obj_ref.last_part != zone_id:
                continue
