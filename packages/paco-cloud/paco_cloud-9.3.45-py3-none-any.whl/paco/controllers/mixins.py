from paco.aws_api.awslambda.code import update_lambda_code, update_lambda_layer
...
class LambdaDeploy():
    ...
    def lambda_deploy_command(self, resource):
        ...
        if schemas.ILambda.providedBy(resource):
            ...
            layer_arn = None
            if resource.code.layer_zipfile != None:
                print(f"Uploading layer for Lambda '{resource.name}' ({function_name})")
                layer_arn = update_lambda_layer(resource, function_name, resource.code.layer_zipfile, account_ctx, resource.region_name)
            print(f"Uploading code for Lambda '{resource.name}' ({function_name})")
            update_lambda_code(resource, function_name, resource.code.zipfile, account_ctx, resource.region_name)
