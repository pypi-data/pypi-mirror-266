...
def update_lambda_code(resource, function_name, src, account_ctx, aws_region):
    ...
    artifact_name = f'{resource.paco_ref_parts}'
...
def update_lambda_layer(resource, function_name, src, account_ctx, aws_region):
    "Update Lambda function layer"
    lambda_client = account_ctx.get_aws_client('lambda', aws_region)
    artifact_name = f'{resource.paco_ref_parts}.layer'
    if not src.endswith('.zip'):
        src_dir = Path(src)
        zip_output, md5_hash = create_zip_artifact(artifact_name, src_dir)
    else:
    ...
    lambda_client.publish_layer_version(
        LayerName=f'{function_name}-Layer',
