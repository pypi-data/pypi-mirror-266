...
class ECSCapacityProviderClient():
    ...
    def create(self):
        ...
        while True:
            try:
                self.ecs_client.create_capacity_provider(
                ...
                if error.response['Error']['Code'] == 'UpdateInProgressException':
                    ...
                    print(f'!! Waiting for Capcity Manager {self.capacity_provider.aws_name}')
                    time.sleep(1)
                    continue
                else:
                    raise error
    ...
    def update(self, capacity_info):
        ...
        if self.asg_arn != capacity_info['autoScalingGroupProvider']['autoScalingGroupArn']:
            ...
            # Add the Capacity Provider if it is not attached
            existing_cps = self.get_existing_capacity_providers()
            if len(existing_cps) == 0:
                retry_provider_put = True
                while retry_provider_put:
                    try:
                        self.ecs_client.put_cluster_capacity_providers(
                        ...
                        retry_provider_put = False
                    except ClientError as error:
                        if error.response['Error']['Message'].find('is in the process of being updated') == -1:
                            print("Waiting for Capacity provider is finish updating.")
                            time.sleep(5)
                        else:
                            retry_provider_put = False
