...
def override_codestar_notification_rule(hook):
    ...
    .. code-block:: python
        ...
        paco.extend.add_codestar_notification_rule_hook(override_codestar_notification_rule)
...
def override_codepipeline_codedeploy_action(hook):
    """
    Add a hook to change CodePipeline's CodeDeploy action configuration.
    This can be used to proxy CodeDeploy deploying through your own custom
    Lambda function instead of directly by CodePipeline.

    .. code-block:: python

        def override_codepipeline_codedeploy_action():
            return troposphere.codepipeline.Actions()

        paco.extend.add_codepipeline_codedeploy_action_hook(...)

    """
    if paco.models.registry.CODEPIPELINE_CODEDEPLOY_ACTION_HOOK != None:
        raise LoaderRegistrationError(f"Only one Service can override CodePipeline CodeDeploy Action Hooks.")

    paco.models.registry.CODEPIPELINE_CODEDEPLOY_ACTION_HOOK = hook

# CodeDeploy Tempalte
def override_codepipeline_codedeploy_cf_template_action(hook):
    """
    Add a hook to change CodePipeline's CodeDeploy's CF Template configuration.
    This can be used to proxy CodeDeploy deploying through your own custom
    Lambda function instead of directly by CodePipeline.

    .. code-block:: python

        def override_codepipeline_codedeploy_cf_template_action():
            return

        paco.extend.add_codepipeline_codedeploy_action_hook(...)

    """
    if paco.models.registry.CODEPIPELINE_CODEDEPLOY_CF_TEMPLATE_HOOK != None:
        raise LoaderRegistrationError(f"Only one Service can override CodePipeline CodeDeploy CF Template Hooks.")

    paco.models.registry.CODEPIPELINE_CODEDEPLOY_CF_TEMPLATE_HOOK = hook

# CodeDeploy CodePipeline Init stage
def override_codepipeline_codedeploy_init_stage_action(hook):
    """
    Add a hook to change CodePipeline's CodeDeploy's Init Stage configuration.
    This can be used to proxy CodeDeploy deploying through your own custom
    Lambda function instead of directly by CodePipeline.

    .. code-block:: python

        def override_codepipeline_codedeploy_init_stage_action():
            return []

        paco.extend.override_codepipeline_codedeploy_init_stage_action(...)

    """
    if paco.models.registry.CODEPIPELINE_CODEDEPLOY_INIT_STAGE_HOOK != None:
        raise LoaderRegistrationError(f"Only one Service can override CodePipeline CodeDeploy Init Stage Hooks.")

    paco.models.registry.CODEPIPELINE_CODEDEPLOY_INIT_STAGE_HOOK = hook

# CodeDeploy CodePipeline Init stage
def override_codepipeline_codedeploy_stack_hooks_action(hook):
    """
    Add a hook to change CodePipeline CodeDeploy's Stack Hooks.
    This can be used to proxy CodeDeploy deploying through your own custom
    Lambda function instead of directly by CodePipeline.

    .. code-block:: python

        def override_codepipeline_codedeploy_stack_hooks_action():
            return stack_hooks

        paco.extend.add_codepipeline_codedeploy_stack_hook(...)

    """
    if paco.models.registry.CODEPIPELINE_CODEDEPLOY_UPDATE_DEPLOYMENT_GROUP_HOOK != None:
        raise LoaderRegistrationError(f"Only one Service can override CodePipeline CodeDeploy Stack Hooks Hook.")

    paco.models.registry.CODEPIPELINE_CODEDEPLOY_UPDATE_DEPLOYMENT_GROUP_HOOK = hook

# S3 Hooks
def override_s3_notifications_action(hook):
    """
    Add a hook to manage S3 Notifications.

    .. code-block:: python

        def override_s3_notifications_action(resource):
            return

        paco.extend.override_s3_notifications_action(...)

    """
    if paco.models.registry.S3_NOTIFICATIONS_HOOK != None:
        raise LoaderRegistrationError(f"Only one Service can override S3 Notification Hooks.")

    paco.models.registry.S3_NOTIFICATIONS_HOOK = hook

def pre_s3_bucket_policy_action(hook):
    """
    Add a hook to manage S3 Bucket Policies.

    .. code-block:: python

        def pre_s3_bucket_policy_action(resource):
            return

        paco.extend.pre_s3_bucket_policy_action(...)

    """
    if paco.models.registry.PRE_S3_BUCKET_POLICY_HOOK != None:
        raise LoaderRegistrationError(f"Only one Service can override Pre S3 Bucket Policy Hooks.")

    paco.models.registry.PRE_S3_BUCKET_POLICY_HOOK = hook
