        ...
        while True:
            try:
                ...
                if error.response['Error']['Code'] == 'ExpiredToken':
                    ...
                    print(f"paco_buckets: create_bucket: S3 Client Token Expired: Retry number {retry_count}")
                    if retry_count > 10:
                        print(f"paco_buckets: create_bucket: S3 client token refresh tried too many times ({retry_count}), aborting.")
                        raise error
                    continue
                else:
                    raise error
        ...
        while True:
            try:
                ...
                if error.response['Error']['Code'] == 'ExpiredToken':
                    ...
                    if retry_count > 10:
                        print(f"paco_buckets: is_bucket_created: S3 client token refresh tried too many times ({retry_count}), aborting.")
                        raise error
                    continue
