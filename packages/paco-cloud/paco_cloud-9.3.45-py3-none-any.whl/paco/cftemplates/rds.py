...
class RDSAurora(StackTemplate):
    ...
    def __init__(self, stack, paco_ctx,):
        ...
        # Disable MasterUsername and MasterUserPassword if restoring from snapshot or global cluster
        if rds_aurora.global_cluster_identifier != None or rds_aurora.db_snapshot_identifier != None:
            rds_aurora.master_username = None
            rds_aurora.secrets_password = None
        ...
        if db_snapshot_id_enabled == False and rds_aurora.global_cluster_identifier == None:
        ...
        if rds_aurora.global_cluster_identifier != None:
            if references.is_ref(rds_aurora.global_cluster_identifier):
                global_cluster_param = self.create_cfn_parameter(
                    param_type='String',
                    name='GlobalClusterIdentifierParam',
                    description='Global Cluster Identifier for DB Cluster',
                    value=rds_aurora.global_cluster_identifier + '.global_cluster_identifier',
                )
                db_cluster_dict['GlobalClusterIdentifier'] = troposphere.Ref(global_cluster_param)
            else:
                db_cluster_dict['GlobalClusterIdentifier'] = rds_aurora.global_cluster_identifier
        ...
        if rds_aurora.global_cluster_identifier == None and rds_aurora.global_cluster != None and rds_aurora.global_cluster.is_enabled():
            global_cluster_res = troposphere.rds.GlobalCluster(
                title='GlobalCluster',
                template=self.template,
                # GlobalClusterIdentifier=rds_aurora.global_cluster.logical_id,
                # EngineVersion=rds_aurora.engine_version,
                # Engine=rds_aurora.engine
                SourceDBClusterIdentifier=troposphere.Ref(db_cluster_res)
            )
            global_cluster_res.DependsOn = db_cluster_res
            self.create_output(
                title='GlobalClusterIdentifier',
                value=troposphere.Ref(global_cluster_res),
                description='Global Cluster Identifier for DB Cluster',
                ref=rds_aurora.paco_ref_parts + ".global_cluster_identifier",
            )
    ...
    def __init__(self, stack, paco_ctx,):
        ...
        if db_snapshot_id_enabled == False:
            ...
            if rds_config.secrets_password:
                ...
                db_instance_dict['MasterUsername'] = troposphere.Join(
                    '',
                    ['{{resolve:secretsmanager:', troposphere.Ref(secret_arn_param), ':SecretString:username}}' ]
                )
