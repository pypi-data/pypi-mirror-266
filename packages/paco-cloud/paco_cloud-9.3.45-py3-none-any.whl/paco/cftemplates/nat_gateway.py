...
from paco.core.exception import PacoErrorCode, PacoException
...
class NATGateway(StackTemplate):
    ...
    def get_eip_refs(self):
        for eip_ref in self.resource.eips:
            if eip_ref in self.eip_cache:
                continue
            self.eip_cache.append(eip_ref)
            return (eip_ref+'.allocation_id', eip_ref+'.address')
        return (None, None)
    ...
    def ec2_nat_gateway(self, network_config, nat_sg_config, nat_sg_config_ref, nat_config):
        ...
        for az_idx in range(1, network_config.availability_zones+1):
            ...
            if nat_az == 'all' or nat_az == str(az_idx):
                ...
                if nat_config.ec2_instance_ami != None:
                    nat_ami_id = nat_config.ec2_instance_ami
                else:
                ...
                if len(nat_config.eips) == 0:
                    ...
                    (eip_alloc_id_ref, eip_address_ref) = self.get_eip_refs()
                    if eip_alloc_id_ref == None:
                        raise PacoException(PacoErrorCode.Unknown, message="ERROR: Unable to get EIP references.")
                    allocation_id_param = self.create_cfn_parameter(
                        name=self.create_cfn_logical_id_join(
                            str_list=['EIPAllocationId', 'AZ', str(az_idx)],
                            camel_case=True ),
                        param_type='String',
                        description='EIP Allocation Id',
                        value=eip_alloc_id_ref,
                    )
                    eip_assoc_res = troposphere.ec2.EIPAssociation(
                        title=self.create_cfn_logical_id_join(
                            str_list=['EIPAssociation', 'AZ', str(az_idx)],
                            camel_case=True ),
                        AllocationId=troposphere.Ref(allocation_id_param),
                        InstanceId=troposphere.Ref(ec2_resource[az_idx])
                    )
                    self.template.add_resource(eip_assoc_res)
...
"""
        ...
        eip_alloc_id_fmt = """
  EIPAllocationId{0[id]:s}:
    Description: The EIP Allocation ID for AZ {0[id]:s}.
    Type: String
"""

        eip_address_fmt = """
  EIPAddress{0[id]:s}:
    Description: The EIP Addressfor AZ {0[id]:s}.
    Type: String
"""
...
"""
  ...
  NATGatewayPublicIp{0[id]:s}:
    Value: !Ref {0[eip_address_resource]:s}
"""
...
"""
        ...
        while True:
            ...
            if len(nat_config.eips) == 0:
                template_table['resources_yaml'] += eip_fmt.format(cf_table)
                cf_table['allocation_id'] = f"AllocationId: !GetAtt [EIP{unique_id}, AllocationId]"
                cf_table['eip_address_resource'] = f"EIP{unique_id}"
            else:
                cf_table['eip_address_resource'] = f"EIPAddress{unique_id}"
                cf_table['allocation_id'] = f"AllocationId: !Ref EIPAllocationId{unique_id}"
                template_table['parameters_yaml'] += eip_alloc_id_fmt.format(cf_table)
                template_table['parameters_yaml'] += eip_address_fmt.format(cf_table)
                (eip_alloc_id_ref, eip_address_ref) = self.get_eip_refs()

                self.set_parameter('EIPAllocationId'+unique_id, eip_alloc_id_ref)
                self.set_parameter('EIPAddress'+unique_id, eip_address_ref)
            template_table['resources_yaml'] += nat_gateway_fmt.format(cf_table)
