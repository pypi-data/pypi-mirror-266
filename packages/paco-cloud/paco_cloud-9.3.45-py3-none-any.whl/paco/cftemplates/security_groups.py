    ...
    def create_group_rules(self, sg_group_id, sg_name, sg_config, template):
        ...
        for sg_rule_type in ['Ingress', 'Egress']:
            ...
            for sg_rule_config in sg_rule_list:
                ...
                if sg_rule_config.cidr_ip != None and sg_rule_config.cidr_ip != '':
                    if references.is_ref(sg_rule_config.cidr_ip):
                        cidr_ref = references.Reference(sg_rule_config.cidr_ip)
                        rule_dict['CidrIp'] = cidr_ref.resolve(self.paco_ctx.project)
                    else:
                        rule_dict['CidrIp'] = sg_rule_config.cidr_ip
                    if rule_dict['CidrIp'] == None:
                        breakpoint()
