...
troposphere.ec2.LaunchTemplateData.props['SecurityGroupIds'] = (list, False)
...
class ASG(StackTemplate):
    def __init__(
        ...
        launch_template_dict = {
            'LaunchTemplateData': {
                ...
                'ImageId': troposphere.Ref(instance_ami_param),
                'Monitoring': {
                    'Enabled': asg_config.instance_monitoring,
                },
                'InstanceType': asg_config.instance_type,
        ...
        if asg_config.instance_key_pair != None:
            ...
            launch_template_dict['LaunchTemplateData']['KeyName'] = troposphere.Ref(instance_key_pair_param)
        ...
        if len(asg_config.block_device_mappings) > 0:
            ...
            launch_template_dict['LaunchTemplateData']["BlockDeviceMappings"] = mappings
        ...
        if user_data_script != '':
            ...
            launch_template_dict['LaunchTemplateData']['UserData'] = troposphere.Ref(user_data_script_param)

        if role_profile_arn != None:
            launch_config_dict['IamInstanceProfile'] = role_profile_arn
            launch_template_dict['LaunchTemplateData']['IamInstanceProfile'] = {
                'Arn': role_profile_arn
            }
        ...
        launch_template_res = troposphere.ec2.LaunchTemplate.from_dict(
            'LaunchTemplate',
            launch_template_dict
        )
        launch_template_res = None
        #template.add_resource(launch_template_res)
        ...
        if launch_config_res != None:
            asg_dict['LaunchConfigurationName'] = troposphere.Ref(launch_config_res)
        elif launch_template_res != None:
            asg_dict['LaunchTemplate'] = {
                'LaunchTemplateId': troposphere.Ref(launch_template_res),
                'Version': troposphere.GetAtt(launch_template_res, 'LatestVersionNumber')
            }
        ...
        if launch_template_res != None:
            asg_res.DependsOn = launch_template_res
    ...
    def script_manager_ecr_deploy(self, ecr_deploy_group, asg_dict, asg_config, template):
        ...
        ecs_ecr_deploy_project_policy_res = troposphere.iam.ManagedPolicy(
            title='ECSECRDeploy',
            PolicyDocument=PolicyDocument(
                Version="2012-10-17",
                Statement=policy_statements
        ...
        template.add_resource(ecs_ecr_deploy_project_policy_res)