...
from paco.models.references import is_ref, get_model_obj_from_ref, Reference
import troposphere
...
class CloudFront(StackTemplate):
    def __init__(self, stack, paco_ctx, factory_name, netenv_name, env_name):
        ...
        if cloudfront_config.is_enabled() == True:
            ...
            if cloudfront_config.response_header_policies != None:
                for (policy_name, policy) in cloudfront_config.response_header_policies.items():
                    policy_dict = {
                        'ResponseHeadersPolicyConfig': {
                            'Name': f'{netenv_name}-{env_name}-{self.app_name}-{self.resource_group_name}-{self.resource_name}-{policy_name}',
                        }
                    }
                    if policy.security_headers_config != None:
                        sec_header_config = policy.security_headers_config
                        policy_dict['ResponseHeadersPolicyConfig']['SecurityHeadersConfig'] = {}
                        sec_header_dict = policy_dict['ResponseHeadersPolicyConfig']['SecurityHeadersConfig']
                        if sec_header_config.content_security_policy != None:
                            sec_header_dict['ContentSecurityPolicy'] = {
                                'Override': sec_header_config.content_security_policy.override,
                                'ContentSecurityPolicy': sec_header_config.content_security_policy.response_header_value
                            }
                        if sec_header_config.referrer_policy != None:
                            sec_header_dict['ReferrerPolicy'] = {
                                'Override': sec_header_config.referrer_policy.override,
                                'ReferrerPolicy': sec_header_config.referrer_policy.response_header_value
                            }
                        if sec_header_config.content_type_options != None:
                            sec_header_dict['ContentTypeOptions'] = {
                                'Override': sec_header_config.content_type_options.override,
                            }
                        if sec_header_config.frame_options_policy != None:
                            sec_header_dict['FrameOptions'] = {
                                'Override': sec_header_config.frame_options_policy.override,
                                'FrameOption': sec_header_config.frame_options_policy.response_header_value
                            }
                        if sec_header_config.strict_transport_security_policy != None:
                            sec_header_dict['StrictTransportSecurity'] = {
                                'AccessControlMaxAgeSec': sec_header_config.strict_transport_security_policy.access_control_max_age,
                                'IncludeSubdomains': sec_header_config.strict_transport_security_policy.include_subdomains,
                                'Override': sec_header_config.strict_transport_security_policy.override,
                                'Preload': sec_header_config.strict_transport_security_policy.preload
                            }
                        if sec_header_config.xss_protection_policy != None:
                            sec_header_dict['XSSProtection'] = {
                                'ModeBlock': sec_header_config.xss_protection_policy.mode_block,
                                'Override': sec_header_config.xss_protection_policy.override,
                                'Protection': sec_header_config.xss_protection_policy.protection,
                            }
                            if sec_header_config.xss_protection_policy.report_uri != None:
                                sec_header_dict['XSSProtection']['ReportUri'] = sec_header_config.xss_protection_policy.report_uri

                    response_header_policy_cf_name = self.create_cfn_logical_id_join(
                            ['ResponseHeaderPolicy', policy_name],
                            camel_case=True
                        )
                    response_header_policy_res = troposphere.cloudfront.ResponseHeadersPolicy.from_dict(
                        response_header_policy_cf_name,
                        policy_dict)
                    self.template.add_resource(response_header_policy_res)
                    self.response_header_policies[policy_name] = response_header_policy_res
                    self.create_output(
                        title=response_header_policy_cf_name,
                        value=troposphere.Ref(response_header_policy_res),
                        ref=policy.paco_ref_parts + '.' + policy_name
                    )
        ...
        for lambda_association in cloudfront_config.default_cache_behavior.lambda_function_associations:
            lambda_associations.append({
                ...
                'LambdaFunctionARN': troposphere.Sub(
                    '${FunctionArn}:${Version}',
                    FunctionArn=troposphere.Ref(lambda_params[lambda_association.lambda_function]),
                    Version=lambda_association.lambda_version)
            })
        ...
        response_policy_id = cloudfront_config.default_cache_behavior.response_headers_policy_id
        if response_policy_id != None:
            if is_ref(response_policy_id):
                # TODO: Allow references to other CloudFront resources
                response_policy_ref = Reference(response_policy_id)
                distribution_config_dict['DefaultCacheBehavior']['ResponseHeadersPolicyId'] = troposphere.Ref(self.response_header_policies[response_policy_ref.last_part])
            else:
                distribution_config_dict['DefaultCacheBehavior']['ResponseHeadersPolicyId'] = response_policy_id
        ...
        # Logging
        if cloudfront_config.logging != None and cloudfront_config.logging.enabled == True:
            logging_bucket_param = self.create_cfn_parameter(
              param_type='String',
              name='CloudFrontLoggingBucketName',
              description='CloudFront Logging S3 Bucket',
              value=cloudfront_config.logging.s3_bucket + '.regional_domain'
            )
            distribution_config_dict['Logging'] = {
                'Bucket': troposphere.Ref(logging_bucket_param),
                'IncludeCookies': cloudfront_config.logging.include_cookies
            }
            if cloudfront_config.logging.prefix != None:
                distribution_config_dict['Logging']['Prefix'] = cloudfront_config.logging.prefix
