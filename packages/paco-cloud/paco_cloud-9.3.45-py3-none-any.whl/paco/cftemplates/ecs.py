...
class ECSServices(StackTemplate):
    def __init__(self, stack, paco_ctx, task_execution_role):
        ...
        for task in ecs_config.task_definitions.values():
            ...
            # Task Role
            if task.task_role != None:
                task_role_arn = iam_ctl.role_arn(task.task_role.paco_ref)
                task_dict['TaskRoleArn'] = task_role_arn
            ...
            for container_definition in task.container_definitions.values():
                ...
                # if task_dict['ContainerDefinitions'][index]['Image'] == None:
                #     breakpoint()
        ...
        for service in ecs_config.services.values():
            ...
            if service.vpc_config != None:
                ...
                if len(service.vpc_config.security_groups) > 0:
                    ...
                    service_dict['NetworkConfiguration']['AwsvpcConfiguration']['SecurityGroups'] = troposphere.Ref(security_groups_param)

                if len(service.vpc_config.segments) > 0:
                    ...
                    service_dict['NetworkConfiguration']['AwsvpcConfiguration']['Subnets'] = troposphere.Ref(segment_param)
                cfn_assign_public_ip = 'DISABLED'
                if service.vpc_config.assign_public_ip:
                    cfn_assign_public_ip = 'ENABLED'
                service_dict['NetworkConfiguration']['AwsvpcConfiguration']['AssignPublicIp'] = cfn_assign_public_ip
            ...
            for target_tracking_policy in service.target_tracking_scaling_policies.values():
                ...
                if target_tracking_policy.predefined_metric == None:
                    continue
                # ScalingPolicies
                scaling_policy_dict = {}
                if self.paco_ctx.legacy_flag('ecs_tracking_policy_name_2023_08_29') == True:
                    policy_name = f"{service.name}{target_tracking_policy.name}Policy"
                else:
                    policy_name = f"{self.stack.get_name(self)}{target_tracking_policy.name}Policy"
                scaling_policy_dict['PolicyName'] = self.create_cfn_logical_id(policy_name)
