...
"""
        template_table = {
            'subnet_list': None,
            'az1_inbound_nacl_entries': '',
            'az2_inbound_nacl_entries': '',
            'az3_inbound_nacl_entries': ''
        }
        nacl_entry_fmt = """
  NACLAZ{0[az_idx]:d}InboundEntry{0[rule_idx]:d}:
    Type: AWS::EC2::NetworkAclEntry{0[az_condition]:s}
    Properties:
      CidrBlock: {0[source_cidr]:s}
      Protocol: {0[protocol]:s}
      RuleAction: {0[rule_action]:s}
      RuleNumber: {0[rule_number]:d}
      NetworkAclId: !Ref NACLAZ{0[az_idx]:d}
      PortRange:
        To: {0[to_port]:d}
        From: {0[from_port]:d}
"""
        ...
        for az_idx in range(0, availability_zones):
          ...
          if segment_config.nacl != None and segment_config.nacl.is_enabled():
            rule_idx = 1
            for inbound_rule in segment_config.nacl.inbound_rules:
              nacl_entry_table = {
                'source_cidr': inbound_rule.source_cidr,
                'az_idx': az_idx+1,
                'rule_idx': rule_idx,
                'to_port': inbound_rule.to_port,
                'from_port': inbound_rule.from_port,
                'protocol': inbound_rule.protocol,
                'rule_action': inbound_rule.action.lower(),
                'rule_number': inbound_rule.rule_number,
                'az_condition': ''
              }
              if az_idx > 0:
                nacl_entry_table['az_condition'] = f'\n    Condition: AZ{az_idx+1}Enabled'
              if inbound_rule.port != -1:
                nacl_entry_table['to_port'] = inbound_rule.port
                nacl_entry_table['from_port'] = inbound_rule.port
              template_table[f'az{az_idx+1}_inbound_nacl_entries'] +=  nacl_entry_fmt.format(nacl_entry_table)
              rule_idx += 1
