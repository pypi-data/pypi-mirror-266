...
class S3(StackTemplate):
    def __init__(self, stack, paco_ctx, bucket_context, bucket_policy_only):
        ...
        if bucket_policy_only == False:
            ...
            # Lifecycle Rules
            if len(bucket.lifecycle_rules) > 0:
                lifecycle_rules = []
                for rule in bucket.lifecycle_rules:
                    ...
                    if rule.id != None:
                        lifecycle_rule['Id'] = rule.id
                    lifecycle_rules.append(lifecycle_rule)
                cfn_export_dict['LifecycleConfiguration'] = {
            ...
            self.create_output(
                title=s3_logical_id + 'DomainName',
                value=troposphere.GetAtt(s3_resource, 'DomainName'),
                ref=bucket.paco_ref_parts + '.domain'
            )
            self.create_output(
                title=s3_logical_id + 'RegionalDomainName',
                value=troposphere.GetAtt(s3_resource, 'RegionalDomainName'),
                ref=bucket.paco_ref_parts + '.regional_domain'
            )
