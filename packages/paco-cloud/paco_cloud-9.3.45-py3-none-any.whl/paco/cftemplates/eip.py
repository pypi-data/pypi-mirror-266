...
class EIP(StackTemplate):
    def __init__(self, stack, paco_ctx):
        ...
        if group_name == None:
            group_name = 'eips'
        self.set_aws_name('EIP', group_name, self.resource.name)
        ...
        if hasattr(eip_config, 'allocation_id') == False or eip_config.allocation_id == None:
            eip_ref = Reference(eip_config.paco_ref)
            eip_res_title = 'ElasticIP'
            if eip_ref.type == 'resource':
                eip_res_title = 'VPCElasticIP'
            ...
            eip_address_value = troposphere.Ref(eip_res)
            eip_allocation_id = troposphere.GetAtt(eip_res, 'AllocationId')
        else:
            eip_address_value = eip_config.ip_address
            eip_allocation_id = eip_config.allocation_id
        ...
        # DNS not supported for VPC EIP resources yet
        if schemas.IVPCEIP.providedBy(eip_config) == False:
