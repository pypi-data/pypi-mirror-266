...
class Lambda(StackTemplate):
    def __init__(
        ...
        # Troposphere Lambda Function URL
        # if awslambda.function_url != None:
        #     function_url_dict = {
        #         'AuthType': awslambda.function_url.auth_type
        #     }
        #     if awslambda.function_url.qualifier != None:
        #         function_url_dict['Qualifier'] = awslambda.function_url.qualifier

        #     function_url_res = troposphere.awslambda.LambdaFunctionURl.from_dict(
        #         title='LambdaFunctionURl',
        #         template=self.template,
        #         resource_dict=function_url_dict
        #     )
        #     function_url_res.DependsOn = self.awslambda_resource
        #     self.template.add_resource(function_url_res)

        #     # Lambda Function URL permission
        #     troposphere.awslambda.Permission(
        #         title='FunctionUrlPermission',
        #         template=self.template,
        #         Action="lambda:InvokeFunction",
        #         FunctionName=troposphere.GetAtt(self.awslambda_resource, 'Arn'),
        #         Principal='sns.amazonaws.com',
        #         SourceArn=troposphere.Ref(param_name),
        #         Condition=troposphere.awslambda.Condition(
        #             StringEquals={
    ...
    def prepare_s3bucket_artifact_hook(self, hook, is_zip):
        if self.stack.aws_region not in self.code_bucket_name.keys():
            self.prepare_s3bucket_artifact_cache(hook, is_zip)
        self.awslambda.stack.set_parameter(
            'CodeS3Bucket',
            self.code_bucket_name[self.stack.aws_region],
            ignore_changes=False
        )
        self.awslambda.stack.set_parameter(
            'CodeS3Key',
    ...
    def prepare_s3bucket_artifact_cache(self, hook, is_zip):
        self.code_bucket_name[self.stack.aws_region], self.code_artifact_name, md5_hash = init_lambda_code(
