...
class VPC(StackTemplate):
    def __init__(
        ...
        if vpc_config.private_hosted_zone.enabled == True:
            ...
            # DHCP Options to use this private hosted zone
            # dhcp_options = troposphere.ec2.DHCPOptions(
            #     title='DHCPOptions',
            #     DomainName=troposphere.Ref(internal_domain_name_param),
            #     # DomainNameServers=troposphere.GetAtt(private_zone_res, 'NameServers')
            # )
            # template.add_resource(dhcp_options)

            # dhcp_options_assoc = troposphere.ec2.VPCDHCPOptionsAssociation(
            #     'VPCDHCPOptionsAssociation',
            #     DhcpOptionsId=troposphere.GetAtt(dhcp_options, 'DhcpOptionsId'),
            #     VpcId=troposphere.Ref(vpc_resource)
            # )

            # template.add_resource(dhcp_options_assoc)
