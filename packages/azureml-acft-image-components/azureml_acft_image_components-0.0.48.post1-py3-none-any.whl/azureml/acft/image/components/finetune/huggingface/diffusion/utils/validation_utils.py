...
class AzuremlValidatorMixin(ABC):
    ...
    def __init__(
        self, required_columns: Optional[List[str]] = None, required_column_dtypes: Optional[List[List[str]]] = None
    ):
        """
        Azureml Dataset atleast should have the columns that are present in required_columns.
        The required_columns should confirm to the dtypes present in required_column_dtypes
        ...
        if (
            self.label_column_optional
            and self.label_column is not None
            and self.label_column not in self.dataset.column_names
        ):
    ...
    def remove_null_examples(self) -> None:
        ...
        self.dataset = self.dataset.filter(
            lambda example: all([self._check_if_non_empty(value) for _, value in example.items()])
        )
        post_filter_rows = self.dataset.num_rows
        logger.info(
            f"Null filter - examples before filter: {pre_filter_rows} | examples after filter: {post_filter_rows}"
        )
