...
def _data_type(data):
    ...
    for f in _features:
        ...
        if _ftype == "int64":
            ...
            _fnames = _features[f].feature.names
...
class DataSelector(DataSelectorBaseWidget):
    ...
    def getSelected(self, format="dataset"):
        if format == "dataset":
            ...
            new_features = self.dataOriginal.features.copy()
            new_data = Dataset.from_dict(df_dict)
            new_data = new_data.cast(new_features)
            return new_data
        elif format == "dataframe":
            return pd.DataFrame(self.selected)
        else:
            print("Format " + format + " not recognized. Please use either 'dataset' or 'dataframe'.")
            return
...
class InferenceExplorer(InferenceExplorerBaseWidget):
    ...
    def __run_inference(self, change):
        ...
        self.queries.append({"text":change["new"][0], "predict":newQueryPrediction})
    ...
    def getQueries(self, format="dataset"):
        if format == "dataset":
            df = pd.DataFrame(self.queries)
            df_dict = df.to_dict()

            featuresDict = self.data.features.copy().to_dict()

            if featuresDict["label"]:
                new_features = {"text": featuresDict["text"], "predict": featuresDict["label"]}
            else:
                new_features = {"text": featuresDict["text"], "predict": featuresDict["labels"]}
            new_data = Dataset.from_dict(df_dict)
            new_data = new_data.cast(Features.from_dict(new_features))
