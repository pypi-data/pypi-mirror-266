...
class CephAdapter(WorkspaceAdapter, DefaultStacIO):
    ...
    def _sign_urls_modifier(self, item: pystac.Item, bucket_name, collection_name):
        # Get the signed URL for the item

        object_key = f"{collection_name}/{item.id}.tif"
        signed_url = self.get_signed_url(bucket_name, object_key=object_key)

        # Update the item's href with the signed URL

        item_dict = item.to_dict(transform_hrefs=False)
        item_dict["assets"]["raster-result"]["href"] = signed_url
    ...
    def get_stac_items(
        ...
        if not url_modifier:
            return [
                self._sign_urls_modifier(item, bucket_name, collection_name)
                for item in items
            ]
        else:
            return [
                url_modifier(item, bucket_name, collection_name, **kwargs)
                for item in items
            ]
