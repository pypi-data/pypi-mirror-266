"""Python utilities required by DL."""
...
def serialize_dl_object(instance):
    if instance is None:
        return None
...
def deserialize_dl_object(identifier, module_objects=None,
                             custom_objects=None,
                             printable_module_name='object'):
    if identifier is None:
        return None
    if isinstance(identifier, dict):
        # In this case we are dealing with a DL config dictionary.
        config = identifier
...
def transpose_shape(shape, target_format, spatial_axes):
    ...
    # Example
    ```python
        >>> from dl.utils.generic_utils import transpose_shape
