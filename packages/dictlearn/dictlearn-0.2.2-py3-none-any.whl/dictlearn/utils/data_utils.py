...
from dictlearn.utils.generic_utils import Progbar
...
def get_file(fname,
    ...
    cache_dir `~/.dl`, placed in the cache_subdir `datasets`,
    and given the filename `fname`. The final location of a file
    `example.txt` would therefore be `~/.dl/datasets/example.txt`.
    ...
    # Arguments
        ...
        cache_subdir: Subdirectory under the DL cache dir where the file is
        ...
        cache_dir: Location to store cached files, when None it
            defaults to the [DL Directory](/faq/#where-is-the-dl-configuration-filed-stored).

    # Returns
        Path to the downloaded file
    """  # noqa
    if cache_dir is None:
        if 'DL_HOME' in os.environ:
            cache_dir = os.environ.get('DL_HOME')
        else:
            cache_dir = os.path.join(os.path.expanduser('~'), '.dl')
    if md5_hash is not None and file_hash is None:
        file_hash = md5_hash
        hash_algorithm = 'md5'
    datadir_base = os.path.expanduser(cache_dir)
    if not os.access(datadir_base, os.W_OK):
        datadir_base = os.path.join('/tmp', '.dl')
...
def _hash_file(fpath, algorithm='sha256', chunk_size=65535):
    ...
    ```python
        >>> from dl.utils.data_utils import _hash_file
...
class OrderedEnqueuer(SequenceEnqueuer):
    ...
    # Arguments
        sequence: A `dl.utils.data_utils.Sequence` object.
        ...
        try:
            ...
            if 'generator already executing' in str(e):
                raise RuntimeError(
                    ...
                    "DL requires a thread-safe generator when"
