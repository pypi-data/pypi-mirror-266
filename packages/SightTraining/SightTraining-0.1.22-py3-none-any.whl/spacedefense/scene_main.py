...
class GameMainScene(Scene):
        ...
        if event.type == pygame.JOYBUTTONUP:
            ...
            if event.button == 13:
                ...
                if (
                    ...
                    pygame.event.post(
                        pygame.event.Event(self.MY_FIGHTER_SUPER_FIRE_TIMEREVENT)
                    )
    ...
    def _proc_on_keydown(self):
        ...
        if keys[pygame.K_2]:
            self.my_master_fighter.trace_fire(
                self.my_bullets, [self.ufo_super_bullets,self.ufo_units], self.particles, "level2"
            )
        if keys[pygame.K_3]:
            self.my_master_fighter.trace_fire(
            ...
            for myf in self.my_flight_units:
                ...
                if myf.type == "myf_slave":
                    ...
                    myf.fire(
                        ...
                        target_groups=[self.ufo_super_bullets,self.ufo_units, self.ufo_bullets],
                        particle_group=self.particles,
                    )
    ...
    def _auto_myf_master(self):
        ...
        score_cast = configmap["myf_master"]["super_bullet"]["score_cast"]
        if self.my_score >= score_cast * 3 and self.my_master_fighter.super_fire_delay == 0:
            self.my_master_fighter.super_fire(
                self.my_super_bullets,
                self.ufo_master,
                self.particles,
