"""
This module, celi-framework.embeddor, is tailored for embedding pre-cleaned text data from source documents. It takes the cleaned output from the celi-framework.pre-processor module and applies further text data analysis and embedding processes, making the data ready for machine learning models and data analysis tools. Key functionalities include:

- Summarizing extensive text sections pre-cleaned by celi-framework.pre-processor to ensure efficient embedding.
- Filtering 'filled' dictionaries, cleaned by celi-framework.pre-processor, against a 'flat schema' to ensure only relevant information is embedded.
...
- Automating the embedding process for multiple documents, leveraging the celi-framework.pre-processor's output to create well-structured dataframes for analytical tasks.
...
from celi-framework.experimental.utils.ada import (
    get_openai_embedding_sync_timeouts,
    chunk_text,
)
from celi-framework.utils.token_counters import token_counter_og
from celi-framework.utils.llms import quick_ask
from celi-framework.utils.utils import load_json, save_json
import os
import pandas as pd
...
def create_df_data(
    ...
    This function forms part of the celi-framework.embeddor module, which takes cleaned text data as input,
    typically generated by the celi-framework.pre-processor module, and prepares it for embedding.

    Args:
        dict_dir (str): Directory containing the dictionaries with pre-processed text by celi-framework.pre-processor.
...
def loop_through_dicts_to_df(dict_dir, df_dir, token_counter_func):
    ...
    Each dictionary is assumed to be the output of a text cleaning process, ready for embedding by the celi-framework.embeddor module.
    This function is part of the celi-framework.embeddor module and depends on the output of the celi-framework.pre-processor module.

    Args:
        dict_dir (str): Directory containing cleaned dictionaries prepped by celi-framework.pre-processor.
