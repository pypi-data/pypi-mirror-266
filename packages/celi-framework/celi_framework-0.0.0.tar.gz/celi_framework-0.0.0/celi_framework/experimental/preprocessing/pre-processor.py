"""
celi-framework.pre-processor
This script outlines a comprehensive workflow for processing DOCX documents into a cleaned format that is primed for the embedding process in the celi-framework.embeddor module. It employs a variety of utilities to convert, extract, cleanse, and standardize document content, preparing it for advanced machine learning tasks. The process includes:
...
4. Saving the processed content in JSON format, which is structured and ready for the embedding processes executed by the celi-framework.embeddor module.
...
- `process_files_in_directory`: Coordinates the document processing workflow, ensuring each file is systematically cleaned and ready for the celi-framework.embeddor module.
...
from celi-framework.core.templates import make_cleanup_dict_prompt_template
from celi-framework.utils.utils import (
    load_json,
    save_json,
