...
class Processor:
	...
	async def execute_one_batch_item(self):
		...
		if item:
			...
			if id and t and updated_at:
				...
				out2, err2 = await self.dbc.key_query(query_key = "overwatch.batch.bump_batch_item", params=updated_status, mode = "one", item_type = BumpedBatchItem)
				logger.info(f"OUT2: {pprint.pformat(out2)}")
				#(id2, updated_at2), err2 = await self.db.bump_batch_item(error=failure, updated_at=updated_at, result=result) or (None, None)
				if self.do_log:
	...
	async def execute_one_throttled_batch_item(self):
		...
		class Bob(pydantic.BaseModel):
			...
			created_at:str = ""
			updated_at:str = ""
		item, err = await self.dbc.key_query(query_key = f"{self.batch_sql_base}.book_throttled_batch_item", params=updated_status, mode = "one", item_type = Bob | None)
		if item:
			if self.do_log:
			...
			logger.error(f"No pending items found, {err}")
		return False
