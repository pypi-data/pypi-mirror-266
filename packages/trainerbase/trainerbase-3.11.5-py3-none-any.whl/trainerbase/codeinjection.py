...
class CodeInjection(AbstractCodeInjection):
    ...
    @override
    def inject(self):
        injection_point = self.address.resolve()

        self.original_code = pm.read_bytes(injection_point, len(self.code_to_inject))
        pm.write_bytes(injection_point, self.code_to_inject, len(self.code_to_inject))
        self._injected = True
...
class AllocatingCodeInjection(AbstractCodeInjection):  # pylint: disable=too-many-instance-attributes
    ...
    def inject(self):
        ...
        self.original_code = pm.read_bytes(injection_point, self.original_code_length)

        if pm.is_64_bit:
            jump_to_new_mem = compile_asm(get_long_x64_jump_to_new_mem_code(self.new_memory_address))
            jumper_length = len(jump_to_new_mem)
            jump_back_address = injection_point + jumper_length - 1
            jump_back = compile_asm(get_long_x64_jump_back_code(jump_back_address))
        else:
            jump_to_new_mem = compile_asm(get_simple_jump_code(self.new_memory_address, injection_point))
            jumper_length = len(jump_to_new_mem)
            jump_back_address = injection_point + jumper_length
        ...
        pm.write_bytes(injection_point, jump_to_new_mem, len(jump_to_new_mem))
