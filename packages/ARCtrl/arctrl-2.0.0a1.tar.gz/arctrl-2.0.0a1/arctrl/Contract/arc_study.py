...
def _007CStudyPath_007C__007C(input: Array[str]) -> str | None:
    ...
    def _arrow1780(x: str, y: str, input: Any=input) -> bool:
        return x == y

    if (len(input) == 3) if (not equals_with(_arrow1780, input, None)) else False:
        if input[0] == "studies":
...
def ARCtrl_ArcStudy__ArcStudy_ToCreateContract_6FCE9E49(this: ArcStudy, WithFolder: bool | None=None) -> Array[Contract]:
    ...
    def _arrow1784(__unit: None=None, this: Any=this, WithFolder: Any=WithFolder) -> IEnumerable_1[Contract]:
        def _arrow1782(__unit: None=None) -> IEnumerable_1[Contract]:
            ...
            def _arrow1781(p: str) -> IEnumerable_1[Contract]:
                return singleton(Contract.create_create(p, DTOType(6))) if (p != path) else empty()

            return collect(_arrow1781, folder_fs.ToFilePaths(False))

        def _arrow1783(__unit: None=None) -> IEnumerable_1[Contract]:
            return singleton(c)

        return append(_arrow1782() if with_folder else empty(), delay(_arrow1783))

    return to_array(delay(_arrow1784))
