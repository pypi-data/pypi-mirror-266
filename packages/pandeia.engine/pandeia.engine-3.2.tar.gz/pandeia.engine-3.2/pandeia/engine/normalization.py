...
from .constants import PANDEIA_WAVEUNITS, PANDEIA_FLUXUNITS, UNIT_MAP, MIN_CLIP
...
class Normalization(DefaultConfig):
    ...
    def __init__(self, webapp=False, config={}, **kwargs):
        ...
        **kwargs: keyword/value pairs
        ...
        if self.type in self.types:
            ...
            type_conf = self._load_dict(type_conf, "api", msg=f"No normalization definition for {type(self)}")
    ...
    def _get_config(self):
        ...
        config = self._load_dict(config,"api", msg="No normalization API defined.")
        if self.webapp:
            config.pop("defaults")
        else:
            config = self._load_dict(config, "defaults", msg="No normalization defaults defined.")
