...
@pytest.mark.skipif(
    'secvision_bindeps.zmq.backend.cython' not in sys.modules, reason="Requires cython backend"
)
@pytest.mark.skipif(
...
def test_cython(language_level, request, tmpdir):
    ...
    assert 'secvision_bindeps.zmq.tests.cython_ext' not in sys.modules
    ...
    def unimport():
        ...
        sys.modules.pop('secvision_bindeps.zmq.tests.cython_ext', None)
