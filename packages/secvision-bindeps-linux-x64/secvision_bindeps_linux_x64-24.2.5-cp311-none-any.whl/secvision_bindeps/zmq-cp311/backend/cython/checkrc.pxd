...
cdef inline int _check_rc(int rc) except -1:
    ...
    if rc == -1: # if rc < -1, it's a bug in libzmq. Should we warn?
        if errno == EINTR:
            from secvision_bindeps.zmq.error import InterruptedSystemCall
            raise InterruptedSystemCall(errno)
        elif errno == EAGAIN:
            from secvision_bindeps.zmq.error import Again
            raise Again(errno)
        elif errno == ZMQ_ETERM:
            from secvision_bindeps.zmq.error import ContextTerminated
            raise ContextTerminated(errno)
        else:
            from secvision_bindeps.zmq.error import ZMQError
