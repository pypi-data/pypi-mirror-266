...
class ZMQError(ZMQBaseError):
    ...
    def __init__(self, errno=None, msg=None):
        ...
        msg : string
            Description of the error or None.
        """
        from secvision_bindeps.zmq.backend import strerror, zmq_errno
        if errno is None:
            errno = zmq_errno()
...
class ContextTerminated(ZMQError):
    ...
    def __init__(self, errno='ignored', msg='ignored'):
        from secvision_bindeps.zmq import ETERM
...
class Again(ZMQError):
    ...
    def __init__(self, errno='ignored', msg='ignored'):
        from secvision_bindeps.zmq import EAGAIN
...
def _check_rc(rc, errno=None):
    ...
    if rc == -1:
        if errno is None:
            from secvision_bindeps.zmq.backend import zmq_errno
            errno = zmq_errno()
        from secvision_bindeps.zmq import EAGAIN, ETERM
        if errno == EINTR:
...
class ZMQVersionError(NotImplementedError):
    ...
    def __init__(self, min_version, msg='Feature'):
        ...
        if _zmq_version is None:
            from secvision_bindeps.zmq import zmq_version
...
def _check_version(min_version_info, msg='Feature'):
    ...
    if _zmq_version_info is None:
        from secvision_bindeps.zmq import zmq_version_info
