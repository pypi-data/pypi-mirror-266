    ...
    async def react_mutate(
        ...
        if method == 'Create':
            ...
            raise resemble.aio.aborted.SystemAborted(
                resemble.v1alpha1.errors_pb2.InvalidMethod(),
                message=
                f"Method 'UpTask' is a task that can not be called directly via React (for now)"
            )
        elif method == 'Down':
            ...
            raise resemble.aio.aborted.SystemAborted(
                resemble.v1alpha1.errors_pb2.InvalidMethod(),
                message=
                f"Method 'DownTask' is a task that can not be called directly via React (for now)"
            )
        else:
            logger.warning(
        ...
        *,
        ...
        try:
            ...
            raise TypeError(
                ...
                f"for method '{method_name}' on '{context.actor_id}'"
            ) from e
        ...
        with self._idempotency_manager.idempotently(
            ...
            async with self._call(
                ...
                for (key, value) in await call.trailing_metadata():  # type: ignore[attr-defined]
                    if key == 'x-resemble-task-id-uuid':
                ...
                raise resemble.aio.aborted.SystemAborted(
                    ...
                    message='Trailing metadata missing for direct task call',
                )
    async def Down(
        self,
        request: resemble.cloud.v1alpha1.application.application_pb2.DownRequest,
...
class ApplicationServicer(Servicer):
    ...
    def lookup(
        ...
        *,
        ...
        actor_id: Optional[ActorId] = resemble.aio.internals.contextvars.get_actor_id()
...
resemble.cloud.v1alpha1.application.application_pb2.InvalidInputError        ]
    ...
    async def Up(
        ...
        *,
        container_image_name: Optional[str] = None,
    ) -> tuple[Application.WeakReference, resemble.cloud.v1alpha1.application.application_pb2.UpResponse]:
        if container_image_name is not None and not isinstance(
            container_image_name,
            str,
        ):
        ...
        return (
            reference,
            await reference._writer(__context_or_workflow__).Up(
                request,
                idempotency=idempotency,
                metadata=metadata,
                bearer_token=bearer_token,
    ...
    async def Down(
        ...
        __context_or_workflow__: TransactionContext | Workflow,
        __options__: Optional[resemble.aio.call.Options] = None,
    ) -> tuple[Application.WeakReference, resemble.cloud.v1alpha1.application.application_pb2.DownResponse]:
        # TODO: mypy-protobuf declares that
        # `google.protobuf.message.Message` constructor arguments are
        # always non-None, when in reality they are optional.
        request = resemble.cloud.v1alpha1.application.application_pb2.DownRequest(
        ...
        return (
            reference,
            await reference._writer(__context_or_workflow__).Down(
                request,
                idempotency=idempotency,
                metadata=metadata,
                bearer_token=bearer_token,
        ...
        def schedule(
            ...
            *,
            ...
            class Tasks:
                def __init__(
                    self,
                    writer: Callable[[TransactionContext | Workflow], ApplicationWriterStub],
                    *,
                    when: Optional[datetime | timedelta] = None,
                ):
                    self._writer = writer
                ...
                async def UpTask(
                    ...
                    *,
                    ...
                    return Application.UpTaskFuture(
                        ...
                        task_id=await self._writer(__context_or_workflow__).UpTask(
                            request,
                            idempotency=idempotency,
                            metadata=metadata,
                            bearer_token=bearer_token,
                ...
                async def DownTask(
                    ...
                    return Application.DownTaskFuture(
                        __context_or_workflow__,
                        task_id=await self._writer(__context_or_workflow__).DownTask(
                            request,
                            idempotency=idempotency,
                            metadata=metadata,
                            bearer_token=bearer_token,
            ...
            return Tasks(self._writer, when=when)
