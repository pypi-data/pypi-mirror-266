    ...
    async def react_mutate(
        ...
        if method == 'Send':
            ...
            raise resemble.aio.aborted.SystemAborted(
                resemble.v1alpha1.errors_pb2.InvalidMethod(),
                message=
                f"Method 'SendTask' is a task that can not be called directly via React (for now)"
            )
        else:
            logger.warning(
        ...
        def schedule(
            ...
            *,
            ...
            class Tasks:
                def __init__(
                    self,
                    writer: Callable[[TransactionContext | Workflow], MessageWriterStub],
                    *,
                    when: Optional[datetime | timedelta] = None,
                ):
                    self._writer = writer
                ...
                async def SendTask(
                    ...
                    *,
                    ...
                    return Message.SendTaskFuture(
                        ...
                        task_id=await self._writer(__context_or_workflow__).SendTask(
                            request,
                            idempotency=idempotency,
                            metadata=metadata,
                            bearer_token=bearer_token,
