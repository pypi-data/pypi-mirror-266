    ...
    async def react_mutate(
        ...
        if method == 'Write':
            ...
            raise resemble.aio.aborted.SystemAborted(
                resemble.v1alpha1.errors_pb2.InvalidMethod(),
                message=
                f"Method 'WriteTask' is a task that can not be called directly via React (for now)"
            )
        elif method == 'Read':
            # Invariant here is that users should not have called this
            # directly but only through code generated React
            ...
            raise resemble.aio.aborted.SystemAborted(
                resemble.v1alpha1.errors_pb2.InvalidMethod(),
                message=
                f"Method 'DeleteTask' is a task that can not be called directly via React (for now)"
            )
        else:
            logger.warning(
                "Got a react mutate request with an invalid method name: "
        ...
        def schedule(
            ...
            *,
            ...
            class Tasks:
                def __init__(
                    self,
                    writer: Callable[[TransactionContext | Workflow], SecretWriterStub],
                    *,
                    when: Optional[datetime | timedelta] = None,
                ):
                    self._writer = writer
                ...
                async def WriteTask(
                    ...
                    *,
                    ...
                    return Secret.WriteTaskFuture(
                        ...
                        task_id=await self._writer(__context_or_workflow__).WriteTask(
                            request,
                            idempotency=idempotency,
                            metadata=metadata,
                            bearer_token=bearer_token,
                ...
                async def DeleteTask(
                    ...
                    return Secret.DeleteTaskFuture(
                        __context_or_workflow__,
                        task_id=await self._writer(__context_or_workflow__).DeleteTask(
                            request,
                            idempotency=idempotency,
                            metadata=metadata,
                            bearer_token=bearer_token,
