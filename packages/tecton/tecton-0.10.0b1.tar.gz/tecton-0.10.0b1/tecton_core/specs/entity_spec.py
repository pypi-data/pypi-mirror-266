...
class EntitySpec(tecton_object_spec.TectonObjectSpec):
    ...
    def from_args_proto(cls, proto: entity__args_pb2.EntityArgs) -> "EntitySpec":
        # TODO(jiadong): Refactor this altogether with migrating entity from legacy join keys to typed join keys.
        join_keys = utils.get_tuple_from_repeated_field(proto.join_keys_legacy) or tuple(
            join_key.name for join_key in proto.join_keys
        )
        return cls(
