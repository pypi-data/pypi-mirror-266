...
class Entity(base_tecton_object.BaseTectonObject):
    ...
    def __init__(
        ...
        *,
        ...
        _add_join_keys_to_proto(join_keys, name, args)
    ...
    def join_keys(self) -> List[str]:
        ...
        if self._spec is None:
            return (
                list(self._args.join_keys_legacy)
                if self._args.join_keys_legacy
                else [join_key.name for join_key in self._args.join_keys]
            )
...
def _add_join_keys_to_proto(
    join_keys: Optional[Union[str, List[str], List[Field]]], entity_name: str, proto: entity__args_pb2.EntityArgs
) -> None:
    if not join_keys:
        resolved_join_keys = [entity_name]
    elif isinstance(join_keys, str):
        resolved_join_keys = [join_keys]
    else:
        resolved_join_keys = join_keys

    if _is_list_of_str_join_keys(resolved_join_keys):
        proto.join_keys_legacy.extend(resolved_join_keys)
    elif _is_list_of_field_join_keys(resolved_join_keys):
        resolved_join_keys.sort(key=lambda field: field.name)
        proto.join_keys.extend(
            [
                schema_pb2.Column(name=field.name, offline_data_type=field.dtype.tecton_type.proto)
                for field in resolved_join_keys
            ]
        )
    else:
