...
def compute_aggregate_materialization_schema_from_view_schema(
    ...
    if not is_aggregate:
        if len(feature_view_args.materialized_feature_view_args.embeddings) == 0:
            return view_schema

        embedding_schema = core_schema.Schema.from_dict(
            {
                f.name: tecton_types.ArrayType(tecton_types.Float32Type())
                for f in feature_view_args.materialized_feature_view_args.embeddings
            }
        )
        return (core_schema.Schema(view_schema) + embedding_schema).to_proto()
