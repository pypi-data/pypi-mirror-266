    ...
    def __init__(
        ...
        *,
        ...
        self.__styling = (
            ...
            else RangeStyling(color_spec=styling)
            if isinstance(styling, Palette)
            else RangeStyling(color_spec=color.Single(color=color.Palette.MINT_GREEN))
        )
    ...
    def _custom_validation(self, *, data: pd.DataFrame) -> None:
        ...
        if isinstance(self.data_column, str):
            for row in data.itertuples(index=False):
                for element in getattr(row, self.data_column):
