...
from roktools.helper import add_field_in_numpy_array
...
def parse(filename: str) -> Residuals:
    ...
    data = add_field_in_numpy_array(data, [('epoch', datetime.datetime)])
    data['epoch'] = [from_week_tow(int(row['week']), float(row['tow'])) for row in data]

    data = add_field_in_numpy_array(data, [('processing_direction', 'S8')])
