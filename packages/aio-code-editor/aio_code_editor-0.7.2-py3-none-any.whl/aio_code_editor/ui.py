...
def code_io(sample=''' ''', l=1, r=1, min_raw=5, max_raw=20, download=False, key=None, type_out="python", file_name="st_test.py" ):
    col1, col2 = st.columns([l, r])
    with col1:
        if type_out == "plot":
            codes, res_type, response_dict, line_nums = codes_box(sample, min_raw, max_raw, type_out="plot",file_name=file_name)
        else:
    ...
    with col2:
        if res_type=="" and type_out=="python":
            if line_nums<min_raw:
                ...
                line_nums = max_raw
            if type_out=="plot":
                run_plot(file_name=file_name, codes=codes, res_type=res_type, download=download)
            else:
                code_editor("", buttons=output_btns, height= [line_nums, max_raw] , info=info_bar_output, key=key)

        else:
            if type_out=="python":
                run_code(file_name=file_name, codes=codes, res_type=res_type, response_dict=response_dict, download=download)
            elif type_out=="plot":
                run_plot(file_name=file_name, codes=codes, res_type=res_type, download=download)