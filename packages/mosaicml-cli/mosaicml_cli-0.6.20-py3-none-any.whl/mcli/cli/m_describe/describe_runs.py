...
class MCLIDescribeRunLifecycleDisplay():
    ...
    def print(self):
        ...
        for event in self.model:
            if current_resumption != event.resumption_index:
    ...
    **kwargs,
    ...
    if not yaml_only:
        ...
        for event in run.lifecycle:
            if estimated_times.get(event.resumption_index, None):
                event.estimated_end_time = seconds_to_str(estimated_times[event.resumption_index])
