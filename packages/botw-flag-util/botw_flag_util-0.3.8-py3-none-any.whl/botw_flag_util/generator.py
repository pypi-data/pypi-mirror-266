...
def actor_bool_flag(
    ...
    if init_value > -2:
        flag.init_value = init_value

    if max_value == 0:
        flag.max_value = False
    elif max_value == 1:
        flag.max_value = True

    if min_value == 0:
        flag.min_value = False
    elif min_value == 1:
        flag.min_value = True

    if reset_type > -2:
        flag.reset_type = reset_type
...
def generate_item_flags() -> None:
    ...
    for actor in moddir.rglob("*.sbactorpack"):
        if "Animal_" in actor.name:
            actor_bool_flag(
                f"IsNewPictureBook_{actor.stem}",
                reset_type=0,
                is_save=1
            )
            actor_bool_flag(
                f"IsRegisteredPictureBook_{actor.stem}",
                reset_type=0,
                is_save=1
            )
            actor_s32_flag(
                f"PictureBookSize_{actor.stem}",
                init_value=-1,
                max_value=65536,
                min_value=-1,
                reset_type=0,
