...
which gives an impression of the whole text; after a well-known
console script for a similar purpose, we'll call this the "head" of the text.
The "head" program extracts the first N lines, which doesn't make much sense
for HTML texts; we count characters instead and allow for some "fuzz" to avoid
...
- Some attributes don't make much sense in such an extract:
  - "href" is removed, for example, since the link target might be part of the
    ...
    be viewed.
  - "id" and "name" elements are removed as well; we remove the href attributes
    which might point to them, and "id" attributes might harm the main document
    to contain the extract.
  - Thinking about it, the only attribute we'll keep by default is "class".
...
conversion_keys = set([
    ...
    'attr_whitelist',   # kept attribute names, e.g. ['class']
    ])
...
def _head_kwargs(kw):
    ...
    [('chars', 300), ('ellipsis', '...'), ('fuzz', 30),
     ('parser', 'DUMMYVALUE'),
     ('unknown_opt', 42), ('words', None)]
...
def _done_after_adding(txt, dic):
    ...
    if not txt:
        return (None, None)
    remaining = dic['chars'] - dic['result_length']
    fuzz = dic['fuzz']
    ...
    if remaining < fuzz:
        ...
        newtext += ellipsis
        dic['result_length'] += len(ellipsis)
        return True, newtext


def make_tag_converter(targettags=None, classmasks=None, **kwargs):
    """
    Create function a tag converter to help convert tags.
