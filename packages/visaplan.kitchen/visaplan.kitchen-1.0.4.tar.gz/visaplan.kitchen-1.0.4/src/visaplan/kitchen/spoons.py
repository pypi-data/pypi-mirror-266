...
def generate_image_infos(txt=None, **kwargs):
    ...
    >>> gii('some text', soup="not None")
    Traceback (most recent call last):
      ...
    TypeError: Got both txt (<type 'str'>) and soup (<type 'str'>)!
...
def fix_webdivs(soup, counter=None, ch=SOFT_HYPHEN):
    ...
    >>> soup2 = BeautifulSoup(txt2.decode('utf-8'), 'lxml')
...
def find_headline(elem, func):
    ...
    >>> soup1 = BeautifulSoup(text1.decode('utf-8'), 'lxml')
    >>> ffunc = lambda el: el.name == 'h2'
    >>> print(unicode(find_headline(soup1.ol, ffunc)))
...
def fence_texts(elem, soup, is_block=is_block_element):
    ...
    ...  Im Folgenden ...
    ...  </div></body>'''
    >>> soup = BeautifulSoup(txt.decode('utf-8'), 'html.parser')
