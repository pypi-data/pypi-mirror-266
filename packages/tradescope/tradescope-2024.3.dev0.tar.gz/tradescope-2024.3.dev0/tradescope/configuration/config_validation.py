...
from tradescope.configuration.deprecated_settings import process_deprecated_setting
from tradescope.enums import RunMode, TradingMode
...
def _extend_validator(validator_class):
    """
    Extended validator for the Tradescope configuration JSON Schema.
...
TradescopeValidator = _extend_validator(Draft4Validator)
...
def validate_config_schema(conf: Dict[str, Any], preliminary: bool = False) -> Dict[str, Any]:
    ...
    try:
        TradescopeValidator(conf_schema).validate(conf)
        return conf
