...
from tradescope.exchange import Exchange, timeframe_to_prev_date, timeframe_to_seconds
from tradescope.exchange.types import OrderBook
from tradescope.misc import append_candles_to_dataframe
from tradescope.rpc import RPCManager
from tradescope.rpc.rpc_types import RPCAnalyzedDFMsg
...
class DataProvider:
    ...
    def get_required_startup(self, timeframe: str) -> int:
        tradeai_config = self._config.get('tradeai', {})
        if not tradeai_config.get('enabled', False):
            return self._config.get('startup_candle_count', 0)
        else:
            startup_candles = self._config.get('startup_candle_count', 0)
            indicator_periods = tradeai_config['feature_parameters']['indicator_periods_candles']
            # make sure the startupcandles is at least the set maximum indicator periods
            self._config['startup_candle_count'] = max(startup_candles, max(indicator_periods))
            tf_seconds = timeframe_to_seconds(timeframe)
            train_candles = tradeai_config['train_period_days'] * 86400 / tf_seconds
            total_candles = int(self._config['startup_candle_count'] + train_candles)
            logger.info(
                f'Increasing startup_candle_count for tradeai on {timeframe} to {total_candles}')
