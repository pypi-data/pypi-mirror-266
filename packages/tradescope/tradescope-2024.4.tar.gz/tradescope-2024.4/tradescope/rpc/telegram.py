...
from tradescope.constants import DUST_PER_COIN, Config
from tradescope.enums import MarketDirection, RPCMessageType, SignalDirection, TradingMode
from tradescope.exceptions import OperationalException
from tradescope.misc import chunks, plural
from tradescope.persistence import Trade
from tradescope.rpc import RPC, RPCException, RPCHandler
from tradescope.rpc.rpc_types import RPCEntryMsg, RPCExitMsg, RPCOrderMsg, RPCSendMsg
from tradescope.util import dt_humanize, fmt_coin, format_date, round_value
...
class Telegram(RPCHandler):
    ...
    def _add_analyzed_candle(self, pair: str) -> str:
        ...
        if candle_val != 'off':
            if candle_val == 'ohlc':
                analyzed_df, _ = self._rpc._tradescope.dataprovider.get_analyzed_dataframe(
    ...
    async def _help(self, update: Update, context: CallbackContext) -> None:
        ...
        if self._rpc._tradescope.trading_mode != TradingMode.SPOT:
    ...
    async def _version(self, update: Update, context: CallbackContext) -> None:
        ...
        :return: None
        """
        strategy_version = self._rpc._tradescope.strategy.version()
    ...
    async def _show_config(self, update: Update, context: CallbackContext) -> None:
        ...
        :return: None
        """
        val = RPC._rpc_show_config(self._config, self._rpc._tradescope.state)
