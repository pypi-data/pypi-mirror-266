...
from tradescope.constants import DOCS_LINK, Config
from tradescope.data.converter import reduce_dataframe_footprint
from tradescope.exceptions import OperationalException
from tradescope.exchange import timeframe_to_seconds
from tradescope.strategy import merge_informative_pair
...
class TradeaiDataKitchen:
    """
    Class designed to analyze data for a single pair. Employed by the ITradeaiModel class.
    ...
    TradeAI was developed by a group of individuals who all contributed specific skillsets to the
    project.
    ...
    def __init__(
        ...
        self.config = config
        self.tradeai_config: Dict[str, Any] = config["tradeai"]
        ...
        self.live = live
        self.pair = pair
        self.keras: bool = self.tradeai_config.get("keras", False)
        self.set_all_pairs()
        self.backtest_live_models = config.get("tradeai_backtest_live_models", False)
        ...
        if not self.live:
            ...
            if not self.backtest_live_models:
                self.full_timerange = self.create_fulltimerange(
                    self.config["timerange"], self.tradeai_config.get("train_period_days", 0)
                )
                (self.training_timeranges, self.backtesting_timeranges) = self.split_timerange(
                    self.full_timerange,
                    config["tradeai"]["train_period_days"],
                    config["tradeai"]["backtest_period_days"],
                )

        self.data['extra_returns_per_train'] = self.tradeai_config.get('extra_returns_per_train', {})
        if not self.tradeai_config.get("data_kitchen_thread_count", 0):
            self.thread_count = max(int(psutil.cpu_count() * 2 - 2), 1)
        else:
            self.thread_count = self.tradeai_config["data_kitchen_thread_count"]
    ...
    def make_train_test_datasets(
        ...
        """
        feat_dict = self.tradeai_config["feature_parameters"]

        if 'shuffle' not in self.tradeai_config['data_split_parameters']:
            self.tradeai_config["data_split_parameters"].update({'shuffle': False})
        ...
        if self.tradeai_config.get('data_split_parameters', {}).get('test_size', 0.1) != 0:
            (
                train_features,
                test_features,
                train_labels,
                ...
                labels,
                weights,
                **self.config["tradeai"]["data_split_parameters"],
            )
        else:
        ...
        if self.tradeai_config['feature_parameters'].get('reverse_train_test_order', False):
    ...
    def set_weights_higher_recent(self, num_weights: int) -> npt.ArrayLike:
        ...
        """
        wfactor = self.config["tradeai"]["feature_parameters"]["weight_factor"]
    ...
    def get_predictions_to_append(self, predictions: DataFrame,
        ...
        if self.tradeai_config["feature_parameters"].get("DI_threshold", 0) > 0:
    ...
    def create_fulltimerange(self, backtest_tr: str, backtest_period_days: int) -> str:
        ...
        if backtest_timerange.stopts == 0:
            ...
            raise OperationalException("TradeAI backtesting does not allow open ended timeranges. "
    ...
    def check_if_model_expired(self, trained_timestamp: int) -> bool:
        ...
        max_time = self.tradeai_config.get("expiration_hours", 0)
        if max_time > 0:
    ...
    def check_if_new_training_required(
        ...
        timeframes = self.tradeai_config["feature_parameters"].get("include_timeframes")
        ...
        if trained_timestamp != 0:
            ...
            retrain = elapsed_time > self.tradeai_config.get("live_retrain_hours", 0)
            if retrain:
                trained_timerange.startts = int(
                    time - self.tradeai_config.get("train_period_days", 0) * SECONDS_IN_DAY
                )
                ...
                data_load_timerange.startts = int(
                    time
                    - self.tradeai_config.get("train_period_days", 0) * SECONDS_IN_DAY
                    - additional_seconds
                )
    ...
    def set_all_pairs(self) -> None:
        ...
        self.all_pairs = copy.deepcopy(
            self.tradeai_config["feature_parameters"].get("include_corr_pairlist", [])
        )
    ...
    def extract_corr_pair_columns_from_populated_indicators(
        ...
        pairs = self.tradeai_config["feature_parameters"].get("include_corr_pairlist", [])
    ...
    def populate_features(self, dataframe: DataFrame, pair: str, strategy: IStrategy,
        ...
        """
        tfs: List[str] = self.tradeai_config["feature_parameters"].get("include_timeframes")
        ...
        for tf in tfs:
            ...
            for t in self.tradeai_config["feature_parameters"]["indicator_periods_candles"]:
            ...
            for n in range(self.tradeai_config["feature_parameters"]["include_shifted_candles"] + 1):
                if n == 0:
                    continue
    ...
    def use_strategy_to_populate_indicators(  # noqa: C901
        ...
        if not new_version:
            raise OperationalException(
                ...
                f"{DOCS_LINK}/strategy_migration/#tradeai-strategy \n"
                "And the feature_engineering_* documentation: \n"
                f"{DOCS_LINK}/tradeai-feature-engineering/"
                )

        tfs: List[str] = self.tradeai_config["feature_parameters"].get("include_timeframes")
        pairs: List[str] = self.tradeai_config["feature_parameters"].get(
        ...
        corr_pairs: List[str] = self.tradeai_config["feature_parameters"].get(
            "include_corr_pairlist", [])
        ...
        if self.live:
            dataframe = strategy.set_tradeai_targets(dataframe.copy(), metadata=metadata)
    ...
    def get_full_models_path(self, config: Config) -> Path:
        """
        Returns default TradeAI model path
        :param config: Configuration dictionary
        """
        tradeai_config: Dict[str, Any] = config["tradeai"]
        return Path(
            config["user_data_dir"] / "models" / str(tradeai_config.get("identifier"))
        )
    ...
    def buffer_timerange(self, timerange: TimeRange):
        ...
        will be NaN and TradeAI will automatically cut those off of the training
        dataset.
        """
        buffer = self.tradeai_config["feature_parameters"]["buffer_train_data_candles"]
        if buffer:
    ...
    def normalize_data(self, data_dictionary: Dict) -> Dict[Any, Any]:
        ...
        """
        logger.warning(f"Your custom ITradeaiModel relies on the deprecated"
                       ...
                       f"{DOCS_LINK}/strategy_migration/#tradeai-new-data-pipeline "
