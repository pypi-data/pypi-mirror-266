...
from tradescope.tradeai.base_models.BasePyTorchModel import BasePyTorchModel
...
class BasePyTorchClassifier(BasePyTorchModel):
    ...
    under IStrategy.set_tradeai_targets method.
    ...
    def get_class_names(self) -> List[str]:
        if not self.class_names:
            raise ValueError(
                ...
                "set self.tradeai.class_names = ['class a', 'class b', 'class c'] "
                "inside IStrategy.set_tradeai_targets method."
            )
    ...
    def train(
        ...
        if not self.tradeai_info.get("fit_live_predictions_candles", 0) or not self.live:
            dk.fit_labels()
