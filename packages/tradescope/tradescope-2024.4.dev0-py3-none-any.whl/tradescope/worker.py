...
from tradescope.constants import PROCESS_THROTTLE_SECS, RETRY_TIMEOUT, Config
from tradescope.enums import RPCMessageType, State
from tradescope.exceptions import OperationalException, TemporaryError
from tradescope.exchange import timeframe_to_next_date
...
class Worker:
    ...
    def _init(self, reconfig: bool) -> None:
        ...
        self.tradescope = TradescopeBot(self._config)
    ...
    def _worker(self, old_state: Optional[State]) -> State:
        ...
        """
        state = self.tradescope.state

        # Log state transition
        if state != old_state:

            if old_state != State.RELOAD_CONFIG:
                self.tradescope.notify_status(f'{state.name.lower()}')

            logger.info(
                f"Changing state{f' from {old_state.name}' if old_state else ''} to: {state.name}")
            if state == State.RUNNING:
                self.tradescope.startup()

            if state == State.STOPPED:
                self.tradescope.check_for_open_trades()
        ...
        if self._heartbeat_interval:
            ...
            if (now - self._heartbeat_msg) > self._heartbeat_interval:
                version = __version__
                strategy_version = self.tradescope.strategy.version()
    ...
    def _process_stopped(self) -> None:
        self.tradescope.process_stopped()

    def _process_running(self) -> None:
        try:
            self.tradescope.process()
            ...
            self.tradescope.notify_status(
                f'*OperationalException:*\n```\n{tb}```\n {hint}',
                msg_type=RPCMessageType.EXCEPTION
            )

            logger.exception('OperationalException. Stopping trader ...')
            self.tradescope.state = State.STOPPED

    def _reconfigure(self) -> None:
        """
        Cleans up current tradescopebot instance, reloads the configuration and
        replaces it with the new instance
        """
        # Tell systemd that we initiated reconfiguration
        self._notify("RELOADING=1")

        # Clean up current tradescope modules
        self.tradescope.cleanup()

        # Load and validate config and create new instance of the bot
        self._init(True)

        self.tradescope.notify_status('config reloaded')
    ...
    def exit(self) -> None:
        ...
        if self.tradescope:
            self.tradescope.notify_status('process died')
            self.tradescope.cleanup()
