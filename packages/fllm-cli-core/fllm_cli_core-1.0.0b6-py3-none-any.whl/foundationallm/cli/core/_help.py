...
Welcome to FLLM CLI!
---------------------
Use `fllm -h` to see available commands.

Telemetry
---------
The FLLM CLI collects usage data in order to improve your experience.
The data is anonymous and does not include commandline argument values.
The data is collected by FoundationLLM.

You can change your telemetry settings with `fllm configure`.
"""

WELCOME_MESSAGE = r"""

Welcome to the FLLM CLI!

Use `fllm --version` to display the current version.
...
class CLIPrintMixin(CLIHelp):
    ...
    def _print_az_find_message(command):
        indent = 0
        message = 'To search AI knowledge base for examples, use: fllm find "az {}"'.format(command)
...
class AzCliHelp(CLIPrintMixin, CLIHelp):
    ...
    def show_help(self, cli_name, nouns, parser, is_group):
        ...
        from foundationallm.cli.core.util import show_updates_available
    ...
    def _register_help_loaders(self):
        import foundationallm.cli.core._help_loaders as help_loaders
        import inspect
