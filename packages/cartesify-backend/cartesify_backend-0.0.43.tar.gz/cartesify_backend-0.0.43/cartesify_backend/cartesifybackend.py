    ...
    def handle_advance(self, data, rollups_url):
        ...
        try:
            ...
            if 'cartesify' in json_data:
                ...
                if "POST" == method:
                    response = requests.post(url=cartesify_data['fetch']['url'], headers=cartesify_data['fetch']['options']['headers'], json=cartesify_data['fetch']['options']['body'])
                elif "PUT" == method:
                    response = requests.put(url=cartesify_data['fetch']['url'],
                                             headers=cartesify_data['fetch']['options']['headers'],
                                             json=cartesify_data['fetch']['options']['body'])
                elif "DELETE" == method:
                    response = requests.delete(url=cartesify_data['fetch']['url'],
                                            headers=cartesify_data['fetch']['options']['headers'],
                                            json=cartesify_data['fetch']['options']['body'])
