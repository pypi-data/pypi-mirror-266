...
@dataclass(frozen=True)
class AppDataModelWithTitle(AppDataModel):
    title: str
...
def dump_data_model(cls: Type[AppT]) -> Union[AppDataModel, AppDataModelWithTitle]:
    if not getattr(cls, "__numerous_app__", False):
        raise ToolDefinitionError(cls)

    elements = _dump_element_data_models(cls)
    title = getattr(cls, "__title__", None)

    return (
        AppDataModel(name=cls.__name__, elements=elements)
        if title is None
        else AppDataModelWithTitle(title=title, name=cls.__name__, elements=elements)
    )
