...
class Trainer(ABC, object):
    def __init__(
        ...
        logger.debug(
            ...
            f"{len(self.train_dataset.dataset)} train images, "
            f"{len(self.train_dataset)} batches, "
            f"and a batch size of {self.config['batch_size']}."
        )
        if self.val_dataset is not None:
            logger.debug(
                f"Data loaded: there are "
                f"{len(self.val_dataset.dataset)} val images, "
                ...
                for i, (img, *path, lbl) in enumerate(self.val_dataset):
                    ...
                    if (
                        ...
                        visualize_self_attention(
                            ...
                            images=img,
                            n_iter=None,
                            patch_size=self.config["model"]["student"].get(
                                "patch_size", 16
                            ),
