...
def create_mapping ( distm:np.array , cmd:str	= 'max'	,
    ...
    if bUseTDA :
        #
        print ( 'KMAPPER IS NOT A BUILD REQUIREMENT AND THIS DEVELOPMENT IS ON HOLD' )
        #
...
def full_mapping ( adf:pd.DataFrame , jdf:pd.DataFrame ,
	...
	bRemoveCurse:bool = False , bAuxConsensusPCA:bool = True ,
        ...
        consensus_function = lambda x:np.mean(x,0) , consensus_labels:list[str] = None , bDisbandAggregation:bool = True ,
	...
	composition_type:str = 'absolute' , connectome_lambda_function = None ,
	statistical_function = lambda x : ( ( x.T-np.mean(x,1) )/np.std(x,1) ).T , bUseStatisticalFunction:bool = False ,
        bMultifactorMap:bool = False , multifactor_value_label:str = None ) -> tuple[pd.DataFrame] :
    #
    import biocartograph.special	as biox
	...
	'statistical_function' : statistical_function , 'bUseStatisticalFunction:bool' : bUseStatisticalFunction ,
	'bMultifactorMap:bool' : bMultifactorMap , 'multifactor_value_label:str': multifactor_value_label }
            ...
            if 'str' in str(type(adf.index.name)).lower() :
                print ( 'analyte idx str' , '\t=\t [', adf.index.name ,']'   , file=ofile )
            if 'str' in str(type(adf.columns.name)).lower() :
                print ( 'analyte col str' , '\t=\t [', adf.columns.name ,']' , file=ofile )
            if 'str' in str(type(jdf.index.name)).lower() :
                print ( 'journal idx str' , '\t=\t [', jdf.index.name ,']'   , file=ofile )
            if 'str' in str(type(jdf.columns.name)).lower() :
                print ( 'journal col str' , '\t=\t [', jdf.columns.name ,']' , file=ofile )
    ...
    elif bUseStatisticalFunction :
        input_values = statistical_function( adf.values )
    else :
        from impetuous.special import zvals
    ...
    #
    if bMultifactorMap :
        if multifactor_value_label is None :
            multifactor_value_label = ',crd'
        print ( 'WARNING: YOU ARE USING A DEVELOPMENTAL FEATURE' )
        from impetuous.quantification import multifactor_evaluation
        epls_formula		= EPLSformula
        if epls_formula is None :
            epls_formula	= 'f~'+'+'.join(journal_df.index.values)
        vars = set( [v.split('*')[0] for v in list( set( epls_formula.split('~')[-1].split('+') ) ) ])
        rename_idx = { idx:jdx for jdx in list(vars) for idx in jdf.index if idx == jdx.split('C(')[-1].split(')')[0] }
        jjdf = jdf.rename( index=rename_idx )
        multifactor_results = multifactor_evaluation ( adf , jjdf , formula = epls_formula )
        multifactor_results = multifactor_results .rename( columns={ c:c+',crd' for c in multifactor_results.columns.values if not ',' in c})
        if bVerbose :
            print ( multifactor_results )
            print ( multifactor_results.loc[:,[c for c in  multifactor_results.columns if not (',p' in c or ',q' in c or ',r' in c) ]] )
        input_values_f = multifactor_results.loc[:,[c for c in  multifactor_results.columns if (multifactor_value_label in c ) ]]
        res0  = res[0] .rename( columns = {'EPLS.x':'EPLS x,crd','EPLS.y':'EPLS y,crd'})
        if not EPLSformula is None :
            input_values_f = pd.concat ( [input_values_f.T,res0.loc[:,[c for c in res0.columns if multifactor_value_label in c ]].T ] ).T
        if  ',p' in multifactor_value_label or ',q' in multifactor_value_label :
            xm_ = np.min( np.min( input_values_f ) )
            x_m = np.max( np.max( input_values_f ) )
            TOL = 1E-300
            input_values_f = input_values_f .apply(pd.to_numeric).apply( lambda x: -np.log10( (x - xm_ + TOL)/( x_m + TOL ) ) ).values
        elif ',crd' in multifactor_value_label :
            input_values_f = input_values_f .apply( pd.to_numeric ).apply( lambda x : ( x-np.min(x) )/( np.max(x)-np.min(x) ) )
            u , s , vt = np.linalg.svd ( input_values_f , False )
            input_values_f = u*s
        else :
            input_values_f = input_values_f .apply( pd.to_numeric ).values
    #
    MF_f , MF_s = None , None
    if 'covariation' in distance_type or 'coexpression' in distance_type or bPreCompute or 'kurtosis' in distance_type :
        ...
        if bMultifactorMap :
            u , s , vt = np.linalg.svd ( input_quant_s.T , False )
        MF_s = vt.T*s
    ...
    # NOT THE bUseUMAP IS SET TRUE OR A MULTIFACTOR CONSENSUS MAP IS REQUESTED
                ...
                if 'str' in str(type(label)) :
                    if label in jdf.index :
                        ...
                        if bUseGeometricallyCenteredZvalues :
                            ...
                            z_values = gdf.values # zvals(gdf.values)['z']
