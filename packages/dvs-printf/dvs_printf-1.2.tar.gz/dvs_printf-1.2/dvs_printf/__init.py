...
class init:
    ...
    interval:int|float=2,stay:bool=True,getmat:bool|str=False):
        """
...
style (str, optional): Different types of printing animation styles. 
    Defaults to 'typing'.
speed (int | float, optional): Speed of printf animation from 1 to 6. 
    Defaults to 3.
...
getmat (bool | str, optional): Matrix data modifier for data structures like 
    ...
    def speed(self) -> int | float:
        """Speed of printf animation from 1 to 6."""
        return self._speed
    ...
    def set_interval(self, value: int | float) -> None:
        ...
        if (type(value) == int or type(value) == float) and value >= 0:
            ...
            raise ValueError("Interval must be a non-negative number (int or float).")
...
animation styles: [typing, headline, newsline, mid, gunshort, snip,scatter, fire, shiver, \\
