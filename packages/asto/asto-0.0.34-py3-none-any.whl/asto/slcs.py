...
# %% auto 0
__all__ = ['slice2range', 'slc2rng', 'safeslice', 'sdim', 'slice_dimensions', 'slice_dims', 'match_slice_format',
           'guess_slice_format', 'flip_slice_format', 'slcchain', 'slice2list', 'format_slice', 'asslcfmt',
           'tosliceformat', 'tocoordformat', 'trysfmt', 'try2slc', 'asslc', 'asslice', 'makeslc', 'makeslice',
           'makeslcs', 'make_slices', 'asslcs']
...
# %% ../nbs/30_slcs.ipynb 16
def slice2range(slc: slice) -> range:
    '''
    Converts a slice object to a corresponding range object.

    This function takes a slice object, which is often used for indexing or slicing sequences, 
    and converts it into a range object. This can be useful for iterating over the specified 
    slice or understanding the indices it represents.

    Parameters
    ----------
    slc : slice
        The slice object to be converted into a range. The `slice` object can have `start`, `stop`, and `step` attributes, where:
        - `start` is the beginning index of the slice (inclusive).
        - `stop` is the end index of the slice (exclusive).
        - `step` is the distance between each index in the slice.
        If `start` or `step` is None, they are treated as their default values (`start` defaults to 0, and `step` defaults to 1).

    Returns
    -------
    range
        A range object that represents the same sequence of indices as the input slice. If the `step` attribute of the slice is not provided, it defaults to 1. If the `start` attribute is also not provided, it defaults to 0.

    Examples
    --------
    >>> slice2range(slice(1, 10, 2))
    range(1, 10, 2)

    >>> slice2range(slice(5))
    range(0, 5)

    >>> slice2range(slice(1, 5))
    range(1, 5)

    Notes
    -----
    The function directly maps the `start`, `stop`, and `step` attributes of the slice to a `range` object. 
    If `start` or `step` are `None`, the `range` object is created with the default start of 0 and step of 1, 
    which mimics the behavior of slicing operations in Python lists and other sequence types.
    '''
...
@wraps(slice2range)
def slc2rng(slc: slice) -> range:
    '''Alias for `slice2range`.'''
    return slice2range(slc)

# %% ../nbs/30_slcs.ipynb 18
