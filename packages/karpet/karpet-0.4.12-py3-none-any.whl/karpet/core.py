...
class Karpet:
    ...
    def get_quick_search_data(self, symbol_or_slug):
        """
        ...
        :param str symbol_or_slug: Coin symbol or slug to search for.
        ...
        if not self.quick_search_data:
            self.quick_search_data = self._get_json(
                f"https://www.coingecko.com/en/search_v2?query={symbol_or_slug}&vs_currency=usd"
            )["coins"]
    ...
    def fetch_crypto_historical_data(self, symbol=None, slug=None):
        ...
        * total_volume
        ...
        search_data = self.get_quick_search_data(slug)

        if not len(search_data):
            raise Exception("No search data available.")

        found_item = list(filter(lambda item: item["id"] == slug, search_data))
