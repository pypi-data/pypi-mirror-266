...
class DynamodbStorageClient(AbstractStorageClient):
    ...
    def retrieve_by_chunk(self, keys: list[dict[Any, Any]] | None = None, table_items: list[dict] | None = None, request_items: dict[str, dict] | None = None):
        table_items = table_items or []
        if not request_items:
            key_list = [{key_item[0]: self.create_value_type(key_item[1]) for key_item in key_entry.items()} for key_entry in keys]
        ...
        return [self.parse_annotated_response(data_item=data_item) for data_item in table_items]
    ...
    def validate_item_values(self, table_item_dict: dict):
        return {key: self.convert_value(value) for key, value in table_item_dict.items()}
    ...
    def convert_value(self, value: Any):
        match value:
            ...
            case dict():
                return {key: self.convert_value(item_value) for key, item_value in value.items()}
            case _:
                return value

    def get_by_pages(self, page_size: int | None = 100, consistent_read: bool = False, filter_by: dict | None = None) -> Generator[list[dict], None, None]:
