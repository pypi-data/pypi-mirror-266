...
class Step(Div):
    ...
    def render(self):
        ...
        left = self.widget.offset().left
        top = self.widget.offset().top
        width = self.widget.outerWidth() + 28
        if left + width + self.width + 62 > ltk.find("body").width():
            self.opposite = True
            left -= self.width + 62
        else:
            self.opposite = False
    ...
    def add_markers(self):
        ...
        ltk.find("body").append(ltk.Div() # top
        ...
        if self.opposite:
            left = self.widget.offset().left - 32
        else:
            left = self.widget.offset().left + self.widget.outerWidth() - 3
        ltk.find("body").append(ltk.Div() # connector
            .addClass("ltk-step-marker")
            .css("left", left)
