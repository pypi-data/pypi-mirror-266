...
class nnUNetDatasetBlosc2(object):
    ...
    def save_case(data: np.ndarray, seg: np.ndarray, properties: dict, output_filename: str, chunks=None, blocks=None):
        ...
        blosc2.asarray(seg, urlpath=f"{output_filename}_seg.b2nd", chunks=chunks, blocks=blocks)
        write_pickle(properties, output_filename + '.pkl')
...
def convert_nnunet_dataset_to_target_format(dataset: nnUNetDatasetNumpy,
    ...
    tqdmp(convert, dataset.keys(), 8, dataset=dataset, target_dataset_class=target_dataset_class, target_folder=target_folder, chunks=chunks, blocks=blocks)


def convert(k, dataset, target_dataset_class, target_folder, chunks, blocks):
    data, seg, properties = dataset[k]
    data = np.array(data)
    seg = np.array(seg)
    target_dataset_class.save_case(data, seg, properties, join(target_folder, k), chunks, blocks)


if __name__ == '__main__':
    preprocess = True
    name = "blosc2_cAuto"
    load_path = "/home/k539i/Documents/datasets/original/mzz_tests/Dataset1200_AMOS2022_task2"
    result_save_dir = "/home/k539i/Documents/projects/Scifo/evaluation/results/eval_nnunet_read_speed_3d"
    ...
    if preprocess:
            ...
            convert_nnunet_dataset_to_target_format(dataset_numpy, data_class, join(load_path, name), CHUNKS, BLOCKS)
    else:
