...
from paco.models.base import Parent, Name, Named, Deployable, ImportFrom, Enablable
...
@implementer(schemas.ISegmentNetworkACLInboundRule)
class SegmentNetworkACLInbound(Named):
    action = FieldProperty(schemas.ISegmentNetworkACLInboundRule["action"])
    source_cidr = FieldProperty(schemas.ISegmentNetworkACLInboundRule["source_cidr"])
    rule_number = FieldProperty(schemas.ISegmentNetworkACLInboundRule["rule_number"])
    from_port = FieldProperty(schemas.ISegmentNetworkACLInboundRule["from_port"])
    protocol = FieldProperty(schemas.ISegmentNetworkACLInboundRule["protocol"])
    to_port = FieldProperty(schemas.ISegmentNetworkACLInboundRule["to_port"])
    port = FieldProperty(schemas.ISegmentNetworkACLInboundRule["port"])

@implementer(schemas.ISegmentNetworkACL)
class SegmentNetworkACL(Named, Enablable):
    inbound_rules = FieldProperty(schemas.ISegmentNetworkACL["inbound_rules"])
...
class Segment(Named, Deployable):
    ...
    nacl = FieldProperty(schemas.ISegment["nacl"])
