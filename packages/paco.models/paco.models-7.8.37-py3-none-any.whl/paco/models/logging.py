...
from paco.models.base import Enablable
...
class CloudWatchLogGroup(Named, CloudWatchLogRetention, Enablable):
    ...
    log_group_name_prefix = FieldProperty(schemas.ICloudWatchLogGroup["log_group_name_prefix"])
    ...
    def get_full_log_group_name(self):
        prefix = ''
        if self.log_group_name_prefix != None:
            prefix = self.log_group_name_prefix
        if self.external_resource == True:
            return f'{prefix}{self.log_group_name}'
        name = f'{prefix}{self.get_log_group_name()}'
        parent = get_parent_by_interface(self, schemas.ICloudWatchLogSet)
        if parent != None:
            return parent.name + '-' + name
        return name

    def get_arn(self, project):
        "Return the ARN for this Log Group"
        if self.paco_ref_parts.startswith('resource.cloudwatch'):
            parts = self.paco_ref_parts.split('.')
            account_id = Reference(f'paco.ref accounts.{parts[3]}').resolve(project)
            region = parts[4]
            return 'arn:aws:logs:' + region + ':' + account_id + ':log-group:' + self.get_full_log_group_name() + ':*'
        else:
            return None
