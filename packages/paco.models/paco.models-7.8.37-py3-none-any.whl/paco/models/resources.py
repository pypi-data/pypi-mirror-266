...
from paco.models.metrics import Monitorable #, CloudWatchLogGroup
...
class GuardDutyDetectorRegion(Named, Deployable, Monitorable):
    ...
    external_resource = FieldProperty(schemas.IGuardDutyDetectorRegion['external_resource'])
...
@implementer(schemas.ICloudWatchLogsRegion)
class CloudWatchLogsRegion(Named, Deployable, Monitorable):
    log_groups = FieldProperty(schemas.ICloudWatchLogsRegion['log_groups'])

@implementer(schemas.ICloudWatchLogsAccounts)
class CloudWatchLogsAccounts(Named, dict):
    pass

@implementer(schemas.ICloudWatchLogsAccountRegionSets)
class CloudWatchLogsAccountRegionSets(Named, dict):
    pass
...
class CloudWatchResource(Named):
    ...
    logs = FieldProperty(schemas.ICloudWatchResource['logs'])

    def resolve_ref(self, ref):
        return self.resolve_ref_obj.resolve_ref(ref)

# EventBridge
@implementer(schemas.IEventBridgeBusesRule)
class EventBridgeBusesRule(EventsRule):
    pass

@implementer(schemas.IEventBridgeBusesRules)
class EventBridgeBusesRules(Named, dict):
    pass

@implementer(schemas.IEventBridgeBuses)
class EventBridgeBuses(Named, Deployable):
    rules = FieldProperty(schemas.IEventBridgeBuses['rules'])

@implementer(schemas.IEventBridgeRegion)
class EventBridgeRegion(Named, Deployable):
    buses = FieldProperty(schemas.IEventBridgeRegion['buses'])

@implementer(schemas.IEventBridgeAccounts)
class EventBridgeAccounts(Named, dict):
    pass

@implementer(schemas.IEventBridgeAccountRegionSets)
class EventBridgeAccountRegionSets(Named, dict):
    pass

@implementer(schemas.IEventBridgeResource)
class EventBridgeResource(Named):
    services = FieldProperty(schemas.IEventBridgeResource['services'])
...
class IAMUserPermissionCustomPolicy(IAMUserPermission):
    ...
    managed_policy_arns = FieldProperty(schemas.IIAMUserPermissionCustomPolicy['managed_policy_arns'])
...
class IAMUser(Named, Deployable):
    ...
    disable_account_delegates = FieldProperty(schemas.IIAMUser['disable_account_delegates'])
    ...
    managed_policies = FieldProperty(schemas.IIAMUser['managed_policies'])
...
# @implementer(schemas.ILogGroup)
# class LogGroup(Resource, CloudWatchLogGroup):
#     pass