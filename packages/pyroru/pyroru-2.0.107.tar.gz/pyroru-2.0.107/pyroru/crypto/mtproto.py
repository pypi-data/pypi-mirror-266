...
from pyroru.errors import SecurityCheckMismatch
from pyroru.raw.core import Message, Long
from . import aes
...
def unpack(
    ...
    try:
        ...
        if e.args[0] == 0:
            raise ConnectionError(f"Получены пустые данные. Проверьте подключение к Интернету.")
        ...
        raise ValueError(f"Сервер отправил неизвестный конструктор: {hex(e.args[0])}\n{left}")
