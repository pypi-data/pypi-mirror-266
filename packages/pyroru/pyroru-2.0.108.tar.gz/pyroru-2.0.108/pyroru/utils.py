...
from pyroru.file_id import FileId, FileType, PHOTO_TYPES, DOCUMENT_TYPES
...
def get_input_media_from_file_id(
    ...
    try:
        ...
        raise ValueError(
            f'Не удалось декодировать "{file_id}". Значение не представляет существующий локальный файл,'
            f"URL-адрес HTTP или действительный идентификатор файла."
        )

    file_type = decoded.file_type

    if expected_file_type is not None and file_type != expected_file_type:
        raise ValueError(f"Ожидался {expected_file_type.name}, вместо этого получен идентификатор файла {file_type.name}")

    if file_type in (FileType.THUMBNAIL, FileType.CHAT_PHOTO):
        raise ValueError(f"Этот идентификатор файла можно использовать только для загрузки: {file_id}")
    ...
    raise ValueError(f"Неизвестный идентификатор файла: {file_id}")
