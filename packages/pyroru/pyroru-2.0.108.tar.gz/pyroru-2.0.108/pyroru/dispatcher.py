...
class Dispatcher:
    ...
    def __init__(self, client: "pyroru.Client"):
        ...
        async def message_parser(update, users, chats):
            return (
                await pyroru.types.Message._parse(self.client, update.message, users, chats,
        ...
        async def callback_query_parser(update, users, chats):
            return (
                await pyroru.types.CallbackQuery._parse(self.client, update, users),
                CallbackQueryHandler
            )

        async def user_status_parser(update, users, chats):
            return (
                pyroru.types.User._parse_user_status(self.client, update),
                UserStatusHandler
            )

        async def inline_query_parser(update, users, chats):
            return (
                pyroru.types.InlineQuery._parse(self.client, update, users),
                InlineQueryHandler
            )

        async def poll_parser(update, users, chats):
            return (
                pyroru.types.Poll._parse_update(self.client, update),
                PollHandler
            )

        async def chosen_inline_result_parser(update, users, chats):
            return (
                pyroru.types.ChosenInlineResult._parse(self.client, update, users),
                ChosenInlineResultHandler
            )

        async def chat_member_updated_parser(update, users, chats):
            return (
                pyroru.types.ChatMemberUpdated._parse(self.client, update, users, chats),
                ChatMemberUpdatedHandler
            )

        async def chat_join_request_parser(update, users, chats):
            return (
                pyroru.types.ChatJoinRequest._parse(self.client, update, users, chats),
    ...
    async def start(self):
        if not self.client.no_updates:
            ...
            log.info("Запущен %s Обработчик задач", self.client.workers)
    ...
    async def stop(self):
        if not self.client.no_updates:
            ...
            log.info("Остановлен %s Обработчик задач", self.client.workers)
    ...
    def remove_handler(self, handler, group: int):
        async def fn():
            ...
            try:
                if group not in self.groups:
                    raise ValueError(f"Группа {group} не существует. Обработчик не был удален.")
                ...
                async with lock:
                    for group in self.groups.values():
                        for handler in group:
                            ...
                            except pyroru.StopPropagation:
                                raise
                            except pyroru.ContinuePropagation:
                                continue
                            except Exception as e:
                                log.exception(e)
