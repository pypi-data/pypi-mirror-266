...
class SentCode(Object):
    ...
    Parameters:
        type (:obj:`~pyroru.enums.SentCodeType`):
            Type of the current sent code.

        phone_code_hash (``str``):
            Confirmation code identifier useful for the next authorization steps (either
            :meth:`~pyroru.Client.sign_in` or :meth:`~pyroru.Client.sign_up`).

        next_type (:obj:`~pyroru.enums.NextCodeType`, *optional*):
            Type of the next code to be sent with :meth:`~pyroru.Client.resend_code`.

        timeout (``int``, *optional*):
            Delay in seconds before calling :meth:`~pyroru.Client.resend_code`.
    """
