...
class Object:
    def __init__(self, client: "pyroru.Client" = None):
        self._client = client

    def bind(self, client: "pyroru.Client"):
        """Bind a Client instance to this and to all nested Pyroru objects.

        Parameters:
            client (:obj:`~pyroru.types.Client`):
                The Client instance to bind this object with. Useful to re-enable bound methods after serializing and
                deserializing Pyroru objects with ``repr`` and ``eval``.
        """
        self._client = client
