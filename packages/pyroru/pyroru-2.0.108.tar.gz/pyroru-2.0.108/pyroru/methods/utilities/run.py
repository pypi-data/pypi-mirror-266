...
import pyroru
from pyroru.methods.utilities.idle import idle
...
class Run:
    def run(
        ...
        :meth:`~pyroru.Client.start`, :meth:`~pyroru.idle` and :meth:`~pyroru.Client.stop` in sequence.
        ...
        operation. This is almost the same as :py:obj:`asyncio.run` except for the fact that Pyroru's ``run`` uses the
        current event loop instead of a new one.

        If you want to run multiple clients at once, see :meth:`pyroru.compose`.
