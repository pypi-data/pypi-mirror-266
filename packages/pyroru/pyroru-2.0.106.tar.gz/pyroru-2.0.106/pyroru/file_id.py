...
class FileId:
    ...
    def decode(file_id: str):
        ...
        try:
            ...
            raise ValueError(f"Неизвестный тип_файла {тип_файла} для идентификатора_файла {file_id}")
        ...
        if file_type in PHOTO_TYPES:
            ...
            try:
                ...
                raise ValueError(f"Неизвестный источник миниатюр {thumbnail_source} для file_id {file_id}")
...
class FileUniqueId:
    ...
    def decode(file_unique_id: str):
        ...
        try:
            ...
            raise ValueError(f"Неизвестный file_unique_type {file_unique_type} из file_unique_id {file_unique_id}")
        ...
        raise ValueError(f"Неизвестный декодер для file_unique_type {file_unique_type} из file_unique_id {file_unique_id}")
    ...
    def encode(self):
        if self.file_unique_type == FileUniqueType.WEB:
            ...
            raise ValueError(f"Неизвестный кодировщик для file_unique_type {self.file_unique_type}")
