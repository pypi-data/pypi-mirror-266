...
class Client(Methods):
    APP_VERSION = f"PyRU {__version__}"
    DEVICE_MODEL = f"anycorp.dev"
    ...
    async def authorize(self) -> User:
        ...
        print(f"Добро пожаловать в PyRU (version {__version__})")
        print(f"PyRU Бесплатное ПО на основе Pyrogram.\n")

        while True:
            try:
                if not self.phone_number:
                    while True:
                        value = await ainput("Введите номер телефона / Токен бота: ")

                        if not value:
                            continue

                        confirm = (await ainput(f'Это "{value}" верно? (y/N): ')).lower()
        ...
        sent_code_descriptions = {
            enums.SentCodeType.APP: "код в приложении Telegram",
            enums.SentCodeType.SMS: "SMS",
            enums.SentCodeType.CALL: "телефонный звонок",
            enums.SentCodeType.FLASH_CALL: "телефонный экстренный звонок",
            enums.SentCodeType.FRAGMENT_SMS: "Fragment SMS",
            enums.SentCodeType.EMAIL_CODE: "код на эл. почте"
        }

        print(f"Код авторизации был отправлен как: {sent_code_descriptions[sent_code.type]}")

        while True:
            if not self.phone_code:
                self.phone_code = await ainput("Укажите код авторизации: ")
            ...
            try:
                ...
                while True:
                    print("Подсказка пароля: {}".format(await self.get_password_hint()))

                    if not self.password:
                        self.password = await ainput("Укажите пароль 2FA (ничего для восстановления): ", hide=self.hide_password)

                    try:
                        if not self.password:
                            confirm = await ainput("Подтвердите восстановление 2FA (y/n): ")

                            if confirm == "y":
                                email_pattern = await self.send_recovery_code()
                                print(f"Код сброса был отправлен на почту: {email_pattern}")

                                while True:
                                    recovery_code = await ainput("Укажите код сброса: ")
        ...
        while True:
            first_name = await ainput("Укажите имя: ")
            last_name = await ainput("Укажите фамилию (пусто что бы пропустить): ")
    ...
    async def load_session(self):
        ...
        if session_empty:
            if not self.api_id or not self.api_hash:
                raise AttributeError("Ключ API необходим для новых авторизаций. "
                                     "Больше информации ( EN ): https://docs.pyrogram.org/start/auth")
            ...
            if not await self.storage.api_id():
                if self.api_id:
                    ...
                    while True:
                        try:
                            value = int(await ainput("Введите api_id: "))
        ...
        if plugins.get("enabled", True):
            ...
            if not include:
                        ...
                        try:
                            for handler, group in getattr(module, name).handlers:
                                if isinstance(handler, Handler) and isinstance(group, int):
                                    ...
                                    log.info('[{}] [PyRU] {}("{}") в группе {} из "{}"'.format(
                ...
                for path, handlers in include:
                    ...
                    try:
                        ...
                        log.warning('[%s] [PyRU] Игнорирование несуществующего модуля "%s"', self.name, module_path)
                        continue

                    if "__path__" in dir(module):
                        log.warning('[%s] [PyRU] Игнорирование пробелов "%s"', self.name, module_path)
                        continue
                    ...
                    for name in handlers:
                        ...
                        try:
                            ...
                            if warn_non_existent_functions:
                                log.warning('[{}] [PyRU] Игнорирование несуществующей функции "{}" из "{}"'.format(
            ...
            if exclude:
                for path, handlers in exclude:
                    ...
                    if "__path__" in dir(module):
                        log.warning('[%s] [PyRU] Игнорирование пробела "%s"', self.name, module_path)
                        continue
            ...
            if count > 0:
                log.info('[{}] Успешно загружен {} плагин{} из "{}"'.format(
                    self.name, count, "s" if count > 1 else "", root))
            else:
                log.warning('[%s] Не загружен плагин "%s"', self.name, root)
