...
class LruCache:
    ...
    def add_async(self, prefix_key=None, expiration=None):
        """
        Decorator hỗ trợ xử lý trên hàm bất đồng bộ
        # >>> @lru_cache_function(3, 1)
        # ... def f(x):
        # ...    print "Calling f(" + str(x) + ")"
        # ...    return x
        ...
        def wrapper(func):
            return AsyncLRUCachedFunction(func, prefix_key, self.cache, expiration=expiration,
                                          accept_none=self._accept_none)
    ...
    def add_for_class_async(self, prefix_key=None, expiration=None):
        """
        Decorator sừ dụng cho hàm bất đồng bộ, tách riêng để không gây ảnh hưởng đến hàm xử lý cũ
        # >>> @lru_cache_function(3, 1)
        # ... def f(x):
        # ...    print "Calling f(" + str(x) + ")"
        # ...    return x
        ...
        """
        def wrapper(func):
            async def wrapped(my_self, *args, **kwargs):
                me = self
                ...
                try:
                    ...
                    try:
                        value = await func(my_self, *args, **kwargs)
                    except TypeError as e:
                        if 'missing 1 required positional argument' in str(e):
                            raise KeyError('you must use add function')
                        else:
...
class AsyncLRUCachedFunction(object):
    """
    Một chức năng ghi nhớ, được hỗ trợ bởi một bộ nhớ cache LRU.
    Hỗ trợ đối với hàm xử lý bất đồng bộ
    """
    ...
    async def __call__(self, *args, **kwargs):
        ...
        try:
            ...
            try:
                ...
                if isinstance(self.function, staticmethod):
                    value = await self.function.__func__(*args, **kwargs)
                else:
