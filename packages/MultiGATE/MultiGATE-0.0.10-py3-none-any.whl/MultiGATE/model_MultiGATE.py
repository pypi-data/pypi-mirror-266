...
class MGATE():
    ...
    def __init__(self, hidden_dims1, hidden_dims2, spot_num, alpha=0.8, temp = 1.0, nonlinear=True, weight_decay=0.0001):
    ...
    def __call__(self, A, prune_A, GP, X1, X2):
        ...
        if self.alpha == 0:
            ...
            self.Att_l = {x: (1-self.alpha)*self.C1[x] + self.alpha*self.prune_C1[x] for x in self.C1.keys()}
    ...
    def __encoder(self, A, prune_A, H, W, C, prune_C, v, prune_v, layer, k):
        ...
        if self.alpha == 0:
            ...
            prune_C[layer] = self.graph_attention_layer(prune_A, H, prune_v[layer], layer, k)
            return (1 - self.alpha) * tf.sparse_tensor_dense_matmul(C[layer],
    ...
    def __decoder(self, H, W, C, prune_C, layer):
        ...
        if self.alpha == 0:
            ...
            return (1 - self.alpha) * tf.sparse_tensor_dense_matmul(C[layer - 1],
                                                                    H) + self.alpha * tf.sparse_tensor_dense_matmul(
                prune_C[layer - 1], H)
