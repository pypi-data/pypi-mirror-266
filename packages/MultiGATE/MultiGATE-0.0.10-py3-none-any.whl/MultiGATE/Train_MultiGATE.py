...
def train_MultiGATE(adata1, adata2, hidden_dims=[512, 30], bp_width = 10000,alpha=0, temp=1.0, type='ATAC', n_epochs=500, lr=0.0001, key_added='MultiGATE',
...
#    X2 = pd.DataFrame(adata_Vars2.X.toarray()[:, ], index=adata_Vars2.obs.index, columns=adata_Vars2.var.index)
    ...
    trainer = MultiGATE(hidden_dims1=[X1.shape[1]] + hidden_dims, hidden_dims2=[X2.shape[1]] + hidden_dims, spot_num = X1.shape[0], alpha=alpha,
    ...
    if alpha == 0:
        ...
        trainer(G_tf, prune_G_tf, X1, X2)
        embeddings_RNA, embeddings_ATAC, attentions1, attentions2, attentions_gp, loss, ReX_RNA, ReX_ATAC = trainer.infer(G_tf, prune_G_tf, G_gp_tf,
                                                                                                           X1, X2)
