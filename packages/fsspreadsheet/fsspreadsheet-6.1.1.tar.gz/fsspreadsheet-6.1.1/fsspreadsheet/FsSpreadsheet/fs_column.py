...
from ..fable_modules.fable_library.seq import (length, min_by, max_by, map, exists)
    ...
    def create_at(index: int, cells: FsCellsCollection) -> FsColumn:
        def get_index_by(f: Callable[[Callable[[FsCell], int], IEnumerable_1[FsCell]], FsCell]) -> int:
            if length(FsCellsCollection__GetCellsInColumn_Z524259A4(cells, index)) == 0:
                ...
                def _arrow127(c: FsCell, f: Any=f) -> int:
                    return FsAddress__get_RowNumber(c.Address)

                return FsAddress__get_RowNumber(f(_arrow127)(FsCellsCollection__GetCellsInColumn_Z524259A4(cells, index)).Address)


        def _arrow130(projection: Callable[[FsCell], int]) -> Callable[[IEnumerable_1[FsCell]], FsCell]:
            def _arrow129(source_1: IEnumerable_1[FsCell]) -> FsCell:
                class ObjectExpr128:
                    @property
                    def Compare(self) -> Callable[[int, int], int]:
                        return compare_primitives

                return min_by(projection, source_1, ObjectExpr128())

            return _arrow129

        min_row_index: int = get_index_by(_arrow130) or 0
        def _arrow133(projection_1: Callable[[FsCell], int]) -> Callable[[IEnumerable_1[FsCell]], FsCell]:
            def _arrow132(source_2: IEnumerable_1[FsCell]) -> FsCell:
                class ObjectExpr131:
                    @property
                    def Compare(self) -> Callable[[int, int], int]:
                        return compare_primitives

                return max_by(projection_1, source_2, ObjectExpr131())

            return _arrow132

        max_row_index: int = get_index_by(_arrow133) or 0
