...
env_path_dev = "./resources/client/dev.env"
                    ...
                    for commit in range(1, 3):
                        for branch in ["main", "develop"]:
                            for platform in ["win", "mac", "nux", "ios", "android"]:
                                success = artifact.push(env_path_dev, f"{org}", f"{project}", version, branch, sha256(commit), platform, "./resources/client/sample_file.txt")
                    ...
                    for commit in range(1, 3):
                        for branch in ["develop"]:
                            for platform in ["win"]:
                                ...
                                success = artifact.pull(env_path_dev, f"{org}", f"{project}", version, branch, sha256(commit), platform, local_path, f"sample_file.txt")
                                self.assertTrue(success)
        pass
                    ...
                    for commit in range(1, 3):
                        for branch in ["develop"]:
                            for platform in ["win"]:
                                ...
                                if not os.path.exists(local_path):
                                    os.makedirs(local_path)
                                success = artifact.pull(env_path_dev, f"org_{org}", f"project_{project}", version, branch, sha256(commit), platform, local_path, f"sample_file.txt")
                                self.assertTrue(success)
        pass
                    ...
                    for commit in range(1, 3):
                        for branch in ["develop"]:
                            for platform in ["win"]:
                                ...
                                if not os.path.exists(local_path):
                                    os.makedirs(local_path)
                                success = artifact.pull(env_path_dev, f"{org}", f"project_{project}", version, branch, sha256(commit), platform, local_path, f"sample_file.txt")
                                self.assertTrue(success)
        pass

    def test_7_list_valid_access(self):
        url = 'http://127.0.0.1:8000/artifacts/list/files?api_key=73e2cab971d71d2309e16200efd9e77339ee17f63ce43aeb58902a01582a0d1c'
