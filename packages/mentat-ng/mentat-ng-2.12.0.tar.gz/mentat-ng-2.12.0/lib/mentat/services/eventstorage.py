...
def _bq_get_unnested_set(parameters):
    ...
    return psycopg2.sql.Identifier(parameters['aggr_set'])
...
def _bq_timeline(parameters, qname, dbtoplist = None):
    ...
    if is_toplist:
        ...
        subquery = psycopg2.sql.SQL('toplist_with_rest AS (SELECT set::text, sum FROM toplist UNION (SELECT %s as set, total - SUM(sum)::bigint as sum FROM total, toplist GROUP BY total HAVING total - SUM(sum)::bigint > 0) ORDER BY sum DESC)')
        params.append(ST_SKEY_REST)
        subqueries.append(subquery)
