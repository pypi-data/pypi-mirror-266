    ...
    def aggregate_events(events):
        ...
        for ip in events.keys():
            for event in events[ip]:
                ...
                ip_result["count"] += 1
                for ref in jpath_values(event, "Ref"):
                    ip_result["references"][ref] = 1
                ...
                for st in ("Source", "Target"):
                    for k in ("Hostname", "MAC", "Port", "Proto", "URL", "Email", "Ref"):
                        for value in jpath_values(event, st + "." + k):
                            ip_result[st.lower()][k.lower()][value] = 1
                    # Get all (ServiceName, ServiceVersion) tuples. Will be represented as a list in JSON.
                    for section in event.get("Source", []):
                        if section.get("ServiceName"):
                            ip_result[st.lower()]["services"][(section.get("ServiceName"), section.get("ServiceVersion"))] = 1
        for abuse_value in result.values():
            for ip_value in abuse_value.values():
                ...
                ip_value["references"] = list(set(ip_value["references"]))
                for st in ("source", "target"):
                    for k in ("hostname", "mac", "port", "proto", "url", "email", "services", "ref"):
