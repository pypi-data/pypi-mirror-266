...
def get_period(when,period_type='monthly'):
    date=datetime.strptime(parse_date(when), '%Y-%m-%d')
    return ({
        'monthly': date.replace(day=1).strftime("%Y%m"),
        'weekly':  (date - relativedelta(days=date.weekday())).strftime("%Y%m%d"),
