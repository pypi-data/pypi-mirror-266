...
def get_run_name() -> str:
    ...
    if RunStore.id() is None:
        if __logger:
            __logger.warning('get_run_name - has not active runs')
        return ''
...
def init(
    ...
    if __logger:
        __logger.info(f'notebook name: {notebook_name}')
...
def finalize(status: str = "Finished") -> None:
    ...
    if RunStore.status():
        ...
        if active_run is not None:
            if __logger:
                __logger.info(f'finalize run name: {active_run.name}, status: {status}')
            client.update_run(
...
def log_param(key: str, value: Any) -> None:
    ...
    if active_run is not None:
        ...
        if __logger:
            __logger.warning('log_param - has not active run')
        pass
...
def log_params(params: Dict[str, Any]) -> None:
    ...
    if active_run is not None:
        ...
        if __logger:
            __logger.warning('log_params - has not active run')
        pass
...
def log_metric(key: str, value: float, step: int) -> None:
    ...
    if active_run is not None:
        ...
        if __logger:
            __logger.warning('log_metric - has not active run')
        pass
...
def log_metrics(metrics: Dict[str, float], step: int) -> None:
    ...
    if active_run is not None:
        ...
        if __logger:
            __logger.warning('log_metrics - has not active run')
        pass
...
def log_artifact(local_file: str, artifact_path: Optional[str] = None) -> None:
    ...
    if active_run is not None:
        ...
        if __logger:
            __logger.warning('log_artifact - has not active run')
        pass
...
def log_dataset_metadata(name: str, path: str, desc: str = None):
    ...
    if active_run is not None:
        ...
        if __logger:
            __logger.warning('log_dataset_metadata - has not active run')
        pass
...
def log_model_metadata(name: str, path: str, step: int, metric: Optional[dict] = None):
    ...
    if active_run is not None:
        ...
        if __logger:
            __logger.warning('log_model_metadata - has not active run')
        pass
