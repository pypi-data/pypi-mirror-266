...
from relationalai.dsl import Context, Graph, Instance, RelationRef, Vars
...
class Loader(ABC):
    ...
    @abstractmethod
    def load(self, provider: ResourceProvider, model: str, source: ImportSource):
...
def import_file(provider: ResourceProvider, model: str, source: ImportSourceFile, type: LoadType = "auto", **metadata):
