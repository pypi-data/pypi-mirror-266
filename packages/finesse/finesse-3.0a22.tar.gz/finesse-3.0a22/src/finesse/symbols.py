...
def display(a, dunder=False):
    ...
    a : :class:`.Symbol`
        Symbol to print
    dunder : bool
        If True, the names of variables have double underscores pre- and suf-fixing
        the names. Functions are not double underscored.
...
class Symbol(abc.ABC):
    ...
    def lambdify(
        ...
        *args,
        ...
        sym_str = display(expr, dunder=True)
        ...
        for arg in args:
            ...
            if hasattr(arg, "full_name"):
                ...
                sym_str = sym_str.replace(arg.name, ARGS[-1])
        ...
        return eval(f"lambda {','.join(ARGS)}: {sym_str}", _globals)
