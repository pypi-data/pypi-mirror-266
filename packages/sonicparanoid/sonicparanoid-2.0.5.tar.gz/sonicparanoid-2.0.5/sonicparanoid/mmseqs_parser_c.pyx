...
def mmseqs_parser_7flds(alignFile, querySeqLenFile, targetSeqLenFile, outDir=os.getcwd(), outName=None, minBitscore=40, compress:bool=False, complev:int=5, debug=False):
    """
    ...
    cdef int minBitscore_c
    minBitscore_c = <int>minBitscore
    cdef int qlen, slen
...
def dmnd_parser(alignFile, querySeqLenFile, targetSeqLenFile, outDir=os.getcwd(), outName=None, minBitscore=40, compress:bool=False, complev:int=5, debug=False):
    """
    ...
    while True:
        ...
        # Extract query and target sequence IDs
        qid, sid = [x.decode() for x in flds[:2]]
        ''' TODO: remove if errors do not occur
        # this is to avoid the problem due to the conversion adding _0 or _1 to the subject id...
        if qid not in qidLenDict:
            if qid[-2] == "_":
                #print("Query id not found:\t{:s}".format(qid))
        ...
        if tmpHitScore >= minBitscore_c:
            ...
            if currentHitId != prevHitId: #then it is a new hit
                ...
                currentHitDict = OrderedDict() #the ordered dict is better for calculating the overlaps
                #add the hsp to the dictionary
                currentHitDict[f"{qstart}:{qend}"] = (qlen, slen, qstart, qend, sstart, send, tmpHitScore)
                #initialize the scores
                prevHitId = currentHitId
            else: #hsp for the previous hits
                # make sure that the same start and end intervals are not overwritten
