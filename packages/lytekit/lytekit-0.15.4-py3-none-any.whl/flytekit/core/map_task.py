...
class MapPythonTask(PythonTask):
    ...
    def get_custom(self, settings: SerializationSettings) -> Dict[str, Any]:
        custom = ArrayJob(parallelism=self._max_concurrency, min_success_ratio=self._min_success_ratio).to_dict()
        custom.update(self._run_task.get_custom(settings))
        return custom
    ...
    def pre_execute(self, params: ExecutionParameters) -> ExecutionParameters:
        ctx = FlyteContextManager.current_context()
        if ctx.execution_state and ctx.execution_state.mode == ExecutionState.Mode.TASK_EXECUTION:
            return self._run_task.pre_execute(params)
        return None
