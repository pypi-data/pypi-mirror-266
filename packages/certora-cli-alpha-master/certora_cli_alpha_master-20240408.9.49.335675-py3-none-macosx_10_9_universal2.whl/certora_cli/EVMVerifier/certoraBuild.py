    ...
    def _handle_optimize(self, contract_file_path: str, settings_dict: Dict[str, Any],
        ...
        if self.context.solc_optimize_map:
            ...
            if self.context.finder_friendly_optimizer:
                ...
                err_msg = f"--finder_friendly_optimizer option supports solc versions 0.6.7 - 0.8.25 only, got {major}.{minor}.{patch}"
    ...
    def collect_for_file(self,
        ...
        if not is_vyper:
            ...
            if reroute_main_path:
                # Note that self.context.solc_allow_path is either a relative path (relative to the original cwd) or
                # an absolute path which isn't a subpath of the original cwd.
                # In the second case the following line of code is equivalent to `main_path = self.context.solc_allow_path`,
                # and that's the best we can do in that case anyway.
