...
def run_subcommand(argv: list[str]) -> Result[str, str]:
    ...
    if subparser_name == "edit":
        ...
        return out
    elif subparser_name == "eval":
        LOGGER.debug("Running eval subcommand")
        eval_config = core_utils.parse_args(
            main_parser, argv[1:], EvalServerConfig
        )
        LOGGER.debug(f"{eval_config.is_ok()=}")
        out = _eval_editor_servers_with_configs(eval_config, cli_config)
        return out
...
def _eval_editor_servers_with_configs(
    eval_config: Result[EvalServerConfig, str],
    cli_config: Result[AIConfigCLIConfig, str],
) -> Result[str, str]:
    if not (eval_config.is_ok() and cli_config.is_ok()):
        return Err(
            f"Something went wrong with configs: {eval_config=}, {cli_config=}"
        )

    server_outcomes = _run_eval_servers(
        eval_config.unwrap(), cli_config.unwrap().aiconfigrc_path
    )
    if server_outcomes.is_err():
        return Err(f"Something went wrong with servers: {server_outcomes=}")
...
def _run_eval_servers(
    eval_config: EvalServerConfig, aiconfigrc_path: str
) -> Result[list[str], str]:
    ...
    # Must reconstruct, EvalServerConfig is an immutable type (frozen dataclass)
    eval_config_dict = eval_config.model_dump()
    eval_config_dict["server_port"] = port
    eval_config = EvalServerConfig(**eval_config_dict)

    LOGGER.warning(f"Using {port}.")

    # Check if server is already running
    LOGGER.info("Running evaluation servers")
    frontend_procs = (
        _run_frontend_server_background()
        if eval_config.server_mode in [ServerMode.DEBUG_SERVERS]
        else Ok([])
    )
    match frontend_procs:
        case Ok(_):
    ...
    results: list[Result[str, str]] = []
    backend_res = run_backend_server(eval_config, aiconfigrc_path)
    match backend_res:
        case Ok(_):
            pass
        case Err(e):
