...
def update_model() -> FlaskResponse:
    ...
    # UI treats model settings and prompt iteration config separately in state
    # prompt iteration config is set as global model settings when set, but we
    # need to make sure prompt iteration config is included in settings when
    # updating otherwise they would be cleared
    if state.prompt_iteration_config and settings:
        settings.update(state.prompt_iteration_config)
...
@app.route("/api/eval/create_evaluation_set", methods=["POST"])
async def create_evaluation_set() -> FlaskResponse:
    state = get_server_state(app)
    create_evaluation_set_fn = state.evaluation_module.get(
        "create_evaluation_set"
    )
    request_json = request.get_json()

    if create_evaluation_set_fn is None:
        return FlaskResponse(
            (
                {
                    "message": "create_evaluation_set not found in evaluation module"
                },
                500,
            )
        )
        ...
        try:
            eval_set_id = await create_evaluation_set_fn(request_json)
            return FlaskResponse(({"evaluation_set_id": eval_set_id}, 200))
        except Exception as e:
            return FlaskResponse(
                (
                    {
                        "message": f"Error calling create_evaluation_set in evaluation module: {e}",
                    },
                    500,
                )
            )
...
@app.route("/api/eval/evaluation_result_details", methods=["GET"])
async def evaluation_result_details() -> FlaskResponse:
    eval_result_id = request.args.get("evaluation_result_id")
    if eval_result_id is None:
        return FlaskResponse(
            ({"message": "evaluation_result_id not provided"}, 400)
        )

    state = get_server_state(app)
    get_evaluation_result_details_fn = state.evaluation_module.get(
        "get_evaluation_result_details"
    )
    get_evaluation_result_details_columns_fn = state.evaluation_module.get(
        "get_evaluation_result_details_columns"
    )

    if get_evaluation_result_details_fn is None:
        return FlaskResponse(
            (
                {
                    "message": "get_evalation_result_details not found in evaluation module"
                },
                500,
            )
        )
    elif get_evaluation_result_details_columns_fn is None:
        return FlaskResponse(
            (
                {
                    "message": "get_evaluation_result_details_columns not found in evaluation module"
                },
                500,
            )
        )
        ...
        try:
            eval_result_details = await get_evaluation_result_details_fn(
                eval_result_id
            )
            eval_result_details_columns = (
                await get_evaluation_result_details_columns_fn(eval_result_id)
            )

            return FlaskResponse(
                (
                    {
                        "columns": eval_result_details_columns,
            ...
            return FlaskResponse(
                (
                    {
                        "message": f"Error obtaining evaluation_result_details in evaluation module: {e}",
                    },
                    500,
                )
            )
...
@app.route("/api/eval/current_config_file_path", methods=["GET"])
def current_config_file_path() -> FlaskResponse:
    state = get_server_state(app)
    aiconfig = state.aiconfig
    if aiconfig is None:
        return FlaskResponse(
    ...
    return FlaskResponse(({"file_path": aiconfig.file_path}, 200))


@app.route("/api/eval/list_evaluation_data_sources", methods=["GET"])
async def list_evaluation_data_sources() -> FlaskResponse:
    state = get_server_state(app)
    list_evaluation_data_sources_fn = state.evaluation_module.get(
        "list_evaluation_data_sources"
    )
    if list_evaluation_data_sources_fn is None:
        return FlaskResponse(
            (
                {
                    "message": "list_evaluation_data_sources not found in evaluation module"
                },
                500,
            )
        )
        ...
        try:
            eval_data_sources = await list_evaluation_data_sources_fn()
            return FlaskResponse(
                (
                    {"data": eval_data_sources},
                    200,
                )
            )
        except Exception as e:
            return FlaskResponse(
                (
                    {
                        "message": f"Error calling list_evaluation_data_sources in evaluation module: {e}",
                    },
                    500,
                )
            )
...
@app.route("/api/eval/list_evaluation_models", methods=["GET"])
async def list_evaluation_models() -> FlaskResponse:
    state = get_server_state(app)
    list_evaluation_models_fn = state.evaluation_module.get(
        "list_evaluation_models"
    )
    if list_evaluation_models_fn is None:
        return FlaskResponse(
            (
                {
                    "message": "list_evaluation_models not found in evaluation module"
                },
                500,
            )
        )
        ...
        try:
            eval_models = await list_evaluation_models_fn()
            return FlaskResponse(
                (
                    {"data": eval_models},
                    200,
            ...
            return FlaskResponse(
                (
                    {
                        "message": f"Error calling list_evaluation_models in evaluation module: {e}",
                    },
                    500,
                )
            )
...
@app.route("/api/eval/list_evaluation_results", methods=["GET"])
async def list_evaluation_results() -> FlaskResponse:
    eval_set_id = request.args.get("evaluation_set_id")
    if eval_set_id is None:
        return FlaskResponse(
            ({"message": "evaluation_set_id not provided"}, 400)
        )

    state = get_server_state(app)
    list_evaluation_results_fn = state.evaluation_module.get(
        "list_evaluation_results"
    )
    get_evaluation_results_columns_fn = state.evaluation_module.get(
        "get_evaluation_results_columns"
    )

    if list_evaluation_results_fn is None:
        return FlaskResponse(
            (
                {
                    "message": "list_evaluation_results not found in evaluation module"
                },
                500,
            )
        )
    elif get_evaluation_results_columns_fn is None:
        return FlaskResponse(
            (
                {
                    "message": "get_evaluation_results_columns not found in evaluation module"
                },
                500,
            )
        )
        ...
        try:
            eval_results = await list_evaluation_results_fn(eval_set_id)
            eval_results_columns = await get_evaluation_results_columns_fn(
                eval_set_id
            )

            return FlaskResponse(
                (
                    {"columns": eval_results_columns, "data": eval_results},
                    200,
                )
            )
        except Exception as e:
            return FlaskResponse(
                (
                    {
                        "message": f"Error getting evaluation_results in evaluation module: {e}",
                    },
                    500,
                )
            )
...
@app.route("/api/eval/list_evaluation_sets", methods=["GET"])
async def list_evaluation_sets() -> FlaskResponse:
    state = get_server_state(app)
    list_evaluation_sets_fn = state.evaluation_module.get(
        "list_evaluation_sets"
    )
    if list_evaluation_sets_fn is None:
        return FlaskResponse(
            (
                {
                    "message": "list_evaluation_sets not found in evaluation module"
                },
                500,
            )
        )
        ...
        try:
            eval_sets = await list_evaluation_sets_fn()
            return FlaskResponse(
                (
                    {"data": eval_sets},
                    200,
            ...
            return FlaskResponse(
                (
                    {
                        "message": f"Error calling list_evaluation_sets in evaluation module: {e}",
                    },
                    500,
                )
            )
...
@app.route("/api/eval/list_prompt_iteration_data_sources", methods=["GET"])
async def list_prompt_iteration_data_sources() -> FlaskResponse:
    state = get_server_state(app)
    list_prompt_iteration_data_sources_fn = state.evaluation_module.get(
        "list_prompt_iteration_data_sources"
    )
    if list_prompt_iteration_data_sources_fn is None:
        return FlaskResponse(
            (
                {
                    "message": "list_prompt_iteration_data_sources not found in evaluation module"
                },
                500,
            )
        )
        ...
        try:
            eval_data_sources = await list_prompt_iteration_data_sources_fn()
            return FlaskResponse(
                (
                    {"data": eval_data_sources},
                    200,
            ...
            return FlaskResponse(
                (
                    {
                        "message": f"Error calling list_prompt_iteration_data_sources in evaluation module: {e}",
                    },
                    500,
                )
            )
...
@app.route("/api/eval/prompt_iteration_config", methods=["GET"])
async def prompt_iteration_config() -> FlaskResponse:
    state = get_server_state(app)
    return FlaskResponse(
        (
            {"data": state.prompt_iteration_config},
            200,
        )
    )


@app.route("/api/eval/set_prompt_iteration_config", methods=["POST"])
def set_prompt_iteration_config() -> FlaskResponse:
    state = get_server_state(app)
    request_json = request.get_json()
    state.prompt_iteration_config = request_json
    aiconfig = state.aiconfig
    ...
    # Prompt iteration config is accessed in the model parser via regular
    # model settings, so just update the global model settings (for all models)
    # with the new prompt iteration config.
    for model in ModelParserRegistry.parser_ids():
        combined_settings = aiconfig.get_global_settings(model).copy()
        combined_settings.update(request_json)
        aiconfig.update_model(model, settings=combined_settings)

    return FlaskResponse(
        (
            {"message": "Prompt iteration config updated."},
            200,
        )
    )
