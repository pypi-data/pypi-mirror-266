...
class CubeQuery:
    ...
    def order_by(self, *orders: Union[str, OrderSpec]) -> "CubeQuery":
        for order in orders:
            if not isinstance(order, OrderSpec):
                ...
                oname = order.lstrip("-")
                dim = self._resolve_dim_or_agg(oname)
                order = OrderSpec(dimension=dim, direction=direction)
            else:
                order.dimension = self._resolve_dim_or_agg(order.dimension)
    ...
    def _resolve_dim_or_agg(self, dim_or_agg: Union[str, Dimension, Aggregation]):
        try:
            return self._resolve_dim(dim_or_agg)
        except ConfigurationError:
            if dims := [agg for agg in self.aggregations if agg.name == dim_or_agg]:
                return dims[0]
            else:
                raise ConfigurationError(
                    f"Order by '{dim_or_agg}' is not possible - it is neither dimension nor "
                    f"aggregation"
                ) from None
