# coding: utf-8

"""
    CryptoAPIs

    Crypto APIs is a complex and innovative infrastructure layer that radically simplifies the development of any Blockchain and Crypto related applications. Organized around REST, Crypto APIs can assist both novice Bitcoin/Ethereum enthusiasts and crypto experts with the development of their blockchain applications. Crypto APIs provides unified endpoints and data, raw data, automatic tokens and coins forwardings, callback functionalities, and much more.

    The version of the OpenAPI document: 2023-04-25
    Contact: developers@cryptoapis.io
    Generated by OpenAPI Generator (https://openapi-generator.tech)

    Do not edit the class manually.
"""  # noqa: E501


import unittest

from com.cryptoapis.rest_apis.sdk.models.get_asset_details_by_asset_symbol_r import GetAssetDetailsByAssetSymbolR

class TestGetAssetDetailsByAssetSymbolR(unittest.TestCase):
    """GetAssetDetailsByAssetSymbolR unit test stubs"""

    def setUp(self):
        pass

    def tearDown(self):
        pass

    def make_instance(self, include_optional) -> GetAssetDetailsByAssetSymbolR:
        """Test GetAssetDetailsByAssetSymbolR
            include_option is a boolean, when False only required
            params are included, when True both required and
            optional params are included """
        # uncomment below to create an instance of `GetAssetDetailsByAssetSymbolR`
        """
        model = GetAssetDetailsByAssetSymbolR()
        if include_optional:
            return GetAssetDetailsByAssetSymbolR(
                api_version = '2023-04-25',
                request_id = '601c1710034ed6d407996b30',
                context = 'yourExampleString',
                data = com.cryptoapis.rest_apis.sdk.models.get_asset_details_by_asset_symbol_r_data.GetAssetDetailsByAssetSymbolR_data(
                    item = com.cryptoapis.rest_apis.sdk.models.get_asset_details_by_asset_symbol_ri.GetAssetDetailsByAssetSymbolRI(
                        asset_id = '5b1ea92e584bf50020130615', 
                        asset_logo = com.cryptoapis.rest_apis.sdk.models.get_asset_details_by_asset_idri_asset_logo.GetAssetDetailsByAssetIDRI_assetLogo(
                            encoding = 'base64', 
                            image_data = '', 
                            mime_type = 'image/svg+xml', ), 
                        asset_name = 'Bitcoin', 
                        asset_original_symbol = 'BTC', 
                        asset_symbol = 'BTC', 
                        asset_type = 'crypto', 
                        latest_rate = com.cryptoapis.rest_apis.sdk.models.get_asset_details_by_asset_idri_latest_rate.GetAssetDetailsByAssetIDRI_latestRate(
                            amount = '61704.20995670996', 
                            calculation_timestamp = 1636107864, 
                            unit = 'USD', ), 
                        slug = 'bitcoin', 
                        specific_data = com.cryptoapis.rest_apis.sdk.models.get_asset_details_by_asset_symbol_ris.GetAssetDetailsByAssetSymbolRIS(), ), )
            )
        else:
            return GetAssetDetailsByAssetSymbolR(
                api_version = '2023-04-25',
                request_id = '601c1710034ed6d407996b30',
                data = com.cryptoapis.rest_apis.sdk.models.get_asset_details_by_asset_symbol_r_data.GetAssetDetailsByAssetSymbolR_data(
                    item = com.cryptoapis.rest_apis.sdk.models.get_asset_details_by_asset_symbol_ri.GetAssetDetailsByAssetSymbolRI(
                        asset_id = '5b1ea92e584bf50020130615', 
                        asset_logo = com.cryptoapis.rest_apis.sdk.models.get_asset_details_by_asset_idri_asset_logo.GetAssetDetailsByAssetIDRI_assetLogo(
                            encoding = 'base64', 
                            image_data = '', 
                            mime_type = 'image/svg+xml', ), 
                        asset_name = 'Bitcoin', 
                        asset_original_symbol = 'BTC', 
                        asset_symbol = 'BTC', 
                        asset_type = 'crypto', 
                        latest_rate = com.cryptoapis.rest_apis.sdk.models.get_asset_details_by_asset_idri_latest_rate.GetAssetDetailsByAssetIDRI_latestRate(
                            amount = '61704.20995670996', 
                            calculation_timestamp = 1636107864, 
                            unit = 'USD', ), 
                        slug = 'bitcoin', 
                        specific_data = com.cryptoapis.rest_apis.sdk.models.get_asset_details_by_asset_symbol_ris.GetAssetDetailsByAssetSymbolRIS(), ), ),
        )
        """

    def testGetAssetDetailsByAssetSymbolR(self):
        """Test GetAssetDetailsByAssetSymbolR"""
        # inst_req_only = self.make_instance(include_optional=False)
        # inst_req_and_optional = self.make_instance(include_optional=True)

if __name__ == '__main__':
    unittest.main()
