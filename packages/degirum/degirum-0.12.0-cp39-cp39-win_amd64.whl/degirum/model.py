...
from ._filter_models import _check_runtime_device_supported
...
class Model(ABC):
    ...
    def device_type(self, val: str):
        ...
        if val != self.device_type:
            val = val.upper()
            supported_types = self._model_parameters.SupportedDeviceTypes.upper()
            ...
            agent_device = val.split("/")
            if len(agent_device) != 2:
                raise DegirumException(
                    f"Device type must be in format <runtime>/<device>, not '{val}'"
                )

            if not _check_runtime_device_supported(
                agent_device[0], agent_device[1], supported_types
            ):
                raise DegirumException(
                    f"Device type {val} is not supported by model {self._model_name}. "
                    + f"Supported device type are: {supported_types}."
                )
