...
LOG_CODE_NOT_A_FILE_WARNING = "filename %r is not a file; ignoring"
...
USING_ANONYMOUS_MODE = "Comet is now logging experiments in anonymous mode, they will be publicly accessing by using the experiment url"

CLAIMED_ANONYMOUS_USER = "Could not start anonymous mode. If you would like to log data to your Comet account, call `comet_ml.init()`. If you would like to log data to an anonymous account, delete the Comet config file located at %s and call `comet_ml.init(anonymous=True)."

ANONYMOUS_UNALLOWED_ON_PREM = "Anonymous mode is only available on Comet Cloud, please create an account on your Comet instance and call: comet_ml.init(api_key='...')"
...
API_DELETE_TAGS_UNSUPPORTED_BACKEND_VERSION_ERROR = "This feature requires Comet backend version %r or newer. Please contact your Comet Admin."
