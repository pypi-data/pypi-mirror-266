...
def resume_online_experiment(
    ...
    if init_parameters.mode == RESUME_STRATEGY_GET:
        ...
        if exist_in_backend:
            LOGGER.error(
                "Can not create new experiment with key: %r - it is already exists at Comet. Start arguments: %s, %s",
                init_parameters.experiment_key,
                init_parameters,
                effective_config,
            )
            raise InvalidExperimentMode(
                "Can not create new experiment with key: %r - it is already exists at Comet. "
                "Please use %r or %r experiment creation modes."
                % (
                    init_parameters.experiment_key,
                    RESUME_STRATEGY_GET,
                    RESUME_STRATEGY_GET_OR_CREATE,
        ...
        raise InvalidExperimentMode(
            "Unsupported experiment creation mode: %r" % init_parameters.mode
        )
...
def create_online_experiment(
    ...
    if init_parameters.mode == RESUME_STRATEGY_GET:
        LOGGER.error(
            "Can not use online experiment creation mode 'get' without 'experiment_key' value provided. "
            "Start arguments: %s, %s",
            init_parameters,
            effective_config,
        )
...
def create_offline_experiment(
    ...
    if init_parameters.experiment_key is None:
        if init_parameters.is_get():
            LOGGER.error(
                "Can not use offline experiment creation mode 'get' without 'experiment_key' value provided. "
                "Start arguments: %s, %s",
                init_parameters,
                experiment_config,
            )
