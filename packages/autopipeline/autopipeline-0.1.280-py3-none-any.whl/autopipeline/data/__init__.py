...
class QUIET_ML:
    ...
    def query_answer(self, qid):
        ...
        directory = f'data/{qid}'
        dir_path = pkg_resources.files(package_name).joinpath(directory)
        all_files = [f for f in dir_path.iterdir() if f.name.startswith('answer') and f.name.endswith('.csv')]
        return all_files
