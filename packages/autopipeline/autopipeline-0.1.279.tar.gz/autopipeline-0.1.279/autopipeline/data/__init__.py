...
class QUIET_ML:
    ...
    def query_answer(self, qid):
        ...
        # df = pd.read_csv(data_file_path)
        with pkg_resources.path('autopipeline', f'data/{qid}') as dir_path:
            all_files = [f for f in dir_path.iterdir() if f.is_file() and f.name.startswith('answer') and f.name.endswith('.csv')]
        return all_files
