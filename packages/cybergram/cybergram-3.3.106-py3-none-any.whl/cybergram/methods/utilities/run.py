...
import cybergram
from cybergram.methods.utilities.idle import idle
...
class Run:
    def run(
        ...
        :meth:`~cybergram.Client.start`, :meth:`~cybergram.idle` and :meth:`~cybergram.Client.stop` in sequence.
        ...
        operation. This is almost the same as :py:obj:`asyncio.run` except for the fact that cybergram's ``run`` uses the
        current event loop instead of a new one.

        If you want to run multiple clients at once, see :meth:`cybergram.compose`.
