...
class OnDisconnect:
    def on_disconnect(self=None) -> Callable:
        ...
        :obj:`~cybergram.handlers.DisconnectHandler`.
        """

        def decorator(func: Callable) -> Callable:
            if isinstance(self, cybergram.Client):
                self.add_handler(cybergram.handlers.DisconnectHandler(func))
            else:
                if not hasattr(func, "handlers"):
                    func.handlers = []

                func.handlers.append((cybergram.handlers.DisconnectHandler(func), 0))
