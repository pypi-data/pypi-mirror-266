...
class SentCode(Object):
    ...
    Parameters:
        type (:obj:`~cybergram.enums.SentCodeType`):
            Type of the current sent code.

        phone_code_hash (``str``):
            Confirmation code identifier useful for the next authorization steps (either
            :meth:`~cybergram.Client.sign_in` or :meth:`~cybergram.Client.sign_up`).

        next_type (:obj:`~cybergram.enums.NextCodeType`, *optional*):
            Type of the next code to be sent with :meth:`~cybergram.Client.resend_code`.

        timeout (``int``, *optional*):
            Delay in seconds before calling :meth:`~cybergram.Client.resend_code`.
    """
