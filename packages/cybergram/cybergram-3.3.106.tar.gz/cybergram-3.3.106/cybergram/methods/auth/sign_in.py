...
class SignIn:
    async def sign_in(
        ...
        Returns:
            :obj:`~cybergram.types.User` | :obj:`~cybergram.types.TermsOfService` | bool: On success, in case the
            authorization completed, the user is returned. In case the phone number needs to be registered first AND the
            terms of services accepted (with :meth:`~cybergram.Client.accept_terms_of_service`), an object containing
